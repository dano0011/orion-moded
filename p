...
local function Setup(config)
	...
	local function _glowButton(parentOrSection, labelText, callback)
		local sRoot = parentOrSection
		if type(parentOrSection) == "table" then
			sRoot = parentOrSection._Root or parentOrSection.Root or parentOrSection
		end
		if typeof(sRoot) ~= "Instance" then
			return nil
		end
		...
		Container.ZIndex = (sRoot.ZIndex or 0) + 5

...
local Library do
    ...
    do -- Components
        ...
        Library.Sections.AddToggleKeybind = function(self, Data)
            Data = Data or { }
            ...
            local HasKeybind = Data.Keybind ~= nil or Data.DefaultKeybind ~= nil or Data.defaultKeybind ~= nil
            local IsHovered = false
            local UpdateToggleVisuals
            local NewKeybind, KeybindItems
            ...
            local Items = { } do
                ...
                Items["LeftElements"] = Instances:Create("Frame", {
                    Parent = Items["Toggle"].Instance,
                    Name = "\0",
                    BorderColor3 = FromRGB(0, 0, 0),
                    BackgroundTransparency = 1,
                ...
                Instances:Create("UIListLayout", {
                    Parent = Items["LeftElements"].Instance,
                    Name = "\0",
                    VerticalAlignment = Enum.VerticalAlignment.Center,
                    FillDirection = Enum.FillDirection.Horizontal,
                    HorizontalAlignment = Enum.HorizontalAlignment.Left,
                ...
                UpdateToggleVisuals = function()
                    ...
                    local IndicatorWidth = IsCheckbox and 18 or 32
                    local IndicatorTheme = Toggle.Value and "Accent" or (IsHovered and "Hovered Element" or "Element")
                    local LeftWidth = Items["LeftElements"].Instance.AbsoluteSize.X

                    Items["Indicator"].Instance.Size = UDim2New(0, IndicatorWidth, 0, 18)
                    Items["Text"].Instance.Position = UDim2New(0, LeftWidth > 0 and LeftWidth + 6 or 0, 0.5, 0)
                    Items["Indicator"]:ChangeItemTheme({BackgroundColor3 = IndicatorTheme})
                    Items["Indicator"]:Tween(nil, {BackgroundColor3 = Library.Theme[IndicatorTheme]})
                    Items["Text"]:Tween(nil, {TextTransparency = Toggle.Value and 0 or (IsHovered and 0.15 or 0.4)})
                ...
                Items["LeftElements"].Instance:GetPropertyChangedSignal("AbsoluteSize"):Connect(UpdateToggleVisuals)
                Library:RegisterToggleStyleCallback(UpdateToggleVisuals)
            end
            ...
            function Toggle:Set(Bool)
                ...
                Library.Flags[self.Flag] = self.Value
                if NewKeybind then
                    NewKeybind.Toggled = Bool
                    Library.Flags[NewKeybind.Flag] = {
                        Mode = NewKeybind.Mode,
                        Key = NewKeybind.Key,
                        Toggled = NewKeybind.Toggled
                    }
                end
                Library:QueueAutoSave()
                UpdateToggleVisuals()
            ...
            if HasKeybind then
                NewKeybind, KeybindItems = Library:CreateKeybind({
                    Parent = Items["LeftElements"],
            ...
            Items["Toggle"]:Connect("MouseButton1Click", function()
                ...
                if (not KeybindItems or not Library:IsMouseOverFrame(KeybindItems["KeyButton"])) and (Library:IsMouseOverFrame(Items["Text"]) or Library:IsMouseOverFrame(Items["Indicator"]) or Library:IsMouseOverFrame(Items["Toggle"])) then
                    Toggle:Set(not Toggle.Value)
                end
            end)

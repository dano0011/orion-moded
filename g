                ...
                if self.IsOpen then 
                    ...
                    if type(Dropdown.Options) == "function" then
                        OptionsUpdateConnection = RunService.Heartbeat:Connect(function()
                            local Now = os.clock()
                            if Now - LastOptionsRefresh < 0.2 then
                                return
                            end
                            LastOptionsRefresh = Now
        ...
        Library.Sections.AddTextbox = function(self, Data)
            ...
            local Items = { } do
                ...
                Items["Text"] = Instances:Create("TextLabel", {
                    ...
                    FontFace = Library.Font,
                    TextColor3 = FromRGB(255, 255, 255),
                    TextTransparency = 0.3499999940395355,
                    BorderColor3 = FromRGB(0, 0, 0),
                    Text = Textbox.Name,
                    Size = UDim2New(0, 0, 0, 15),
                    BackgroundTransparency = 1,
                ...
                Items["Background"] = Instances:Create("TextButton", {
                    Parent = Items["Textbox"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    Text = "",
                ...
                UpdateTextboxVisuals = function()
                    local IsActive = IsFocused or StringLen(Items["Inline"].Instance.Text) > 0
                    local BackgroundTheme = IsHovered and not IsActive and "Hovered Element" or "Element"
                    local IndicatorTheme = IsActive and "Accent" or (IsHovered and "Hovered Element" or "Element")

                    Items["Background"]:ChangeItemTheme({BackgroundColor3 = BackgroundTheme})
                    Items["Background"]:Tween(nil, {BackgroundColor3 = Library.Theme[BackgroundTheme]})
                    ...
                    Items["Text"]:Tween(nil, {TextTransparency = IsActive and 0 or (IsHovered and 0.15 or 0.35)})
                    Items["Check"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                        ImageTransparency = IsActive and 0 or 1,
                        Size = IsActive and UDim2New(0, 8, 0, 8) or UDim2New(0, 0, 0, 0)
                    })
                end

                Items["Textbox"]:OnHover(function()
                    IsHovered = true
                    UpdateTextboxVisuals()
                end)
                ...
                Items["Background"]:Connect("MouseButton1Down", function()
                    Items["Indicator"]:Tween(TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2New(0, 20, 0, 20)})
                end)

                Items["Background"]:Connect("MouseButton1Up", function()
                    Items["Indicator"]:Tween(TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2New(0, 18, 0, 18)})
                    Items["Inline"].Instance:CaptureFocus()
                end)
    ...
    Library.CreateConfigsSection = function(self, Tab)
        ...
        ConfigsSection:AddButton({
            ...
            Callback = function()
                ...
                local Success, Error = Library:LoadConfig(PastedConfig)
                ...
                Library:MakeNotification({
                    Name = "Success!",
                    Content = "Imported shared config successfully",
                    Time = 3
                })
            end
        })
        ...
        Library._PendingAutoload = function()
            ...
            if Library:IsAutoSaveEnabled() then
                local Success, Error = Library:LoadAutoSaveConfig()

                if Success then
                    AutoSaveLoaded = true
                    Library:MakeNotification({
                        Name = "Config Loaded",
                        Content = "Loaded your last saved config",
                        Time = 3
                    })
                elseif Error then
                    Library:MakeNotification({
                        Name = "Config Loaded",
                        Content = "Failed to load your last saved config: " .. tostring(Error),
                        Time = 5
                    })
                end
            end

...
local Library do
    ...
    Library.ConnectGlobalChat = function(self)
        ...
        local function HandlePacket(Packet)
            ...
            if PacketType == "trial_expire" and type(Packet.trialExpire) == "table" then
                ...
                pcall(function()
                    if self.GetHwidForTrial then
                        LocalHwid = tostring(self.GetHwidForTrial() or "")
                    end
                end)

                if (TargetSenderId ~= "" and TargetSenderId == LocalSenderId)
                    or (TargetHwid ~= "" and LocalHwid ~= "" and TargetHwid == LocalHwid) then
                    task.spawn(function()
                        pcall(function()
    ...
    do
        ...
        local function TriggerTrialExpiryFlow(notificationContent)
            if Library._TrialExpiryHandling or Library._Unloaded then
                return
            end
        ...
        local function GetTrialDefaultMinutes()
            ...
            if cachedMinutes and cachedMinutes > 0 then
                return math.floor(cachedMinutes)
            end
        ...
        local function ShowTrialScreen(trialDurationMinutes, alreadyClaimed)
            ...
            local trialBtnText = alreadyClaimed
                and "Free Trial Used"
                or ("Free Trial " .. formattedTrialDuration)
            ...
            gui, Modal, MsgLabel = BuildTrialPanel({
                ...
                message = alreadyClaimed
                    ...
                    or ("Start your <b>" .. formattedTrialDuration .. "</b> free trial\nto get full access to this script."),
                buttons = {
                    {
                        text = "Exit",
        ...
        task.spawn(function()
            pcall(GetTrialDefaultMinutes)
        end)

        task.spawn(function()
            pcall(CheckTrialStatus)
        end)
    ...
    do -- Components
        Library.MakeWindow = function(self, Data)
            ...
            local PremiumRequired = Data.PremiumRequired == true or configuredTrialMinutes ~= nil
            ...
            if PremiumRequired then
                ...
                if not TrialActive then
                    ...
                    if not TrialApi.TrialManager then
                        return nil
                    end
    ...
    Library.CreateHomeTab = function(self, Window, DiscordLink)
        ...
        local Items = { } do
            ...
            local function CreatePerformanceCard(cardKey, accentKey, valueKey, descriptionKey, options)
                ...
                if options.AccentTheme then
                    Items[accentKey]:AddToTheme({BackgroundColor3 = options.AccentTheme})
                end

                Instances:Create("UICorner", {
                    Parent = Items[accentKey].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(1, 0)
                })
            ...
            local function FormatTrialRemaining(remainingMs)
                ...
                if days > 0 then
                    TableInsert(parts, tostring(days) .. "D")
                end
                if hours > 0 then
                    TableInsert(parts, tostring(hours) .. "H")
                end
                if minutes > 0 or #parts > 0 then
                    TableInsert(parts, tostring(minutes) .. "M")
                end
                if #parts == 0 then
                    TableInsert(parts, tostring(seconds) .. "S")
                end
            ...
            local function UpdateTrialCountdownDisplay(forceRefresh)
                ...
                if TrialVisible and TrialText then
                    Items["TrialTimerValue"].Instance.Text = TrialText
                end

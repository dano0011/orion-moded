...
local Library do
    ...
    do
        ...
        local function ClaimTrialWithHwid(durationHours)
            local hwid = GetHwidForTrial()
            if hwid == "" then
                return false, "Could not get HWID"
            end

            local ok, response, decoded = pcall(function()
                ...
                local response = RequestFunction({
                    Url = Library.TrialManager.HttpUrl .. "/trial/claim",
                    Method = "POST",
                    Headers = {
                        ["Content-Type"] = "application/json",
            ...
            if ok and decoded and decoded.ok then
                Library.TrialManager.Cached = true
                Library.TrialManager.CachedTrialActive = true
                Library.TrialManager.CachedRemainingMs = decoded.remainingMs or (durationHours * 60 * 60 * 1000)
                Library.TrialManager.CachedClaimed = true
                Library.TrialManager.CachedExpired = false
                pcall(UpdatePresence)
                return true, "Trial claimed successfully"
            end

            local checkOk, checkDecoded = pcall(function()
                local payload = HttpService:JSONEncode({
                    hwid = hwid,
                    senderId = tostring(LocalPlayer.UserId),
                    username = tostring(LocalPlayer.Name),
                ...
                local checkResponse = RequestFunction({
                    Url = Library.TrialManager.HttpUrl .. "/trial/check",
                    Method = "POST",
                    Headers = {
                        ["Content-Type"] = "application/json",
                        ["x-chat-token"] = Library.TrialManager.Token
                ...
                if not checkResponse or type(checkResponse.Body) ~= "string" or checkResponse.Body == "" then
                    return nil
                end
                return HttpService:JSONDecode(checkResponse.Body)
            end)

            if checkOk and checkDecoded and checkDecoded.ok and checkDecoded.active then
                Library.TrialManager.Cached = true
                Library.TrialManager.CachedTrialActive = true
                Library.TrialManager.CachedRemainingMs = checkDecoded.remainingMs or (durationHours * 60 * 60 * 1000)
                Library.TrialManager.CachedClaimed = true
                Library.TrialManager.CachedExpired = false
                pcall(UpdatePresence)
        ...
        local function CheckTrialStatus()
            ...
            if hwid == "" then
                ...
                TrialMgr.CachedTrialActive = false
                TrialMgr.CachedClaimed = false
                TrialMgr.CachedExpired = false
            ...
            local ok, result = pcall(function()
                ...
                local response = RequestFunction({
                    Url = TrialMgr.HttpUrl .. "/trial/check",
                    Method = "POST",
                    Headers = {
                        ["Content-Type"] = "application/json",
            ...
            if ok and result and result.ok then
                TrialMgr.Cached = true
                TrialMgr.CachedTrialActive = result.active or false
                TrialMgr.CachedRemainingMs = result.remainingMs or 0
                TrialMgr.CachedClaimed = result.claimed or false
                TrialMgr.CachedExpired = result.expired or false
                pcall(UpdatePresence)
        ...
        local function GetTrialDefaultHours()
            local TrialMgr = Library.TrialManager
            local cachedHours = tonumber(TrialMgr.CachedDefaultHours)
            if cachedHours and cachedHours > 0 then
                return math.floor(cachedHours)
            end
            ...
            if ok and result and result.ok and tonumber(result.durationHours) then
                durationHours = math.max(1, math.floor(tonumber(result.durationHours)))
            end

            TrialMgr.CachedDefaultHours = durationHours
        ...
        local function BuildTrialPanel(panelCfg)
            ...
            for i, bd in ipairs(btns) do
                ...
                local ButtonScale = Instances:Create("UIScale", {
                    Parent = B.Instance
                })
        ...
        local function ShowTrialScreen(trialDurationHours, alreadyClaimed)
            local state = { Claimed = false, Closed = false }
            local Busy = false
            local gui, Modal, MsgLabel

            local trialBtnText = alreadyClaimed
                and "Free Trial Used"
                or ("Free Trial " .. tostring(trialDurationHours) .. "H")

            gui, Modal, MsgLabel = BuildTrialPanel({
                badge = "PREMIUM REQUIRED",
                title = "Premium required",
                message = alreadyClaimed
                    and "You've already used your free trial.\nPurchase premium to regain access."
                    or ("Start your <b>" .. tostring(trialDurationHours) .. " hour</b> free trial\nto get full access to this script."),
                buttons = {
                    {
                        text = "Exit",
                    ...
                    },
                    {
                        ...
                        callback = function()
                            ...
                            task.spawn(function()
                                local success = ClaimTrialWithHwid(trialDurationHours)
                                if success then
                                    state.Claimed = true
                                    state.Closed = true
                                    gui.Instance:Destroy()
        ...
        local function ShowExpiredTrialScreen()
            ...
            local Busy = false
            local gui, Modal, MsgLabel
            local defaultHours = GetTrialDefaultHours()
        ...
        Library.GetTrialDefaultHours = GetTrialDefaultHours
        ...
        task.spawn(function()
            ...
            while Library and not Library._Unloaded do
                ...
                if checkTick >= 30 then
                    checkTick = 0
                    if wasActive then
                        if not (Library and Library.TrialManager) then
                            break
                        end
                        Library.TrialManager.Cached = false
                        ...
                        if ok and claimed and (not active) then
                            ...
                            task.spawn(ShowExpiredTrialScreen)
                            break
                        end
                        ...
                        if Library and Library.TrialManager and Library.TrialManager.CachedTrialActive then
                            wasActive = true
                        end
                    end
                end
    ...
    do -- Components
        Library.MakeWindow = function(self, Data)
            Data = Data or { }
            local PremiumRequired = Data.PremiumRequired or false
            local TrialTime = Data.TrialTime or Library.GetTrialDefaultHours() or 1

            if PremiumRequired then
                local isTrialActive, remainingMs, hasClaimedTrial, isTrialExpired = Library.CheckTrialStatus()
                if not isTrialActive then
                    local alreadyUsed = hasClaimedTrial == true
                    local trialClaimed = Library.ShowTrialModal(TrialTime, alreadyUsed)
                    ...
                    Library.TrialManager.Cached = false
                    isTrialActive, remainingMs = Library.CheckTrialStatus()
                end
            end

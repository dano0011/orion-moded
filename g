...
local settingsTable = {
	...
	System = {
		...
		snow = {Type = 'toggle', Value = false, Name = 'Snow'},
	}
}
...
local Snowflakes = {}
local snowEnabled = getSetting("System", "snow")
local snowRunning = false
local maxSnow = useMobileSizing and 35 or 60
local snowSpawnDelay = useMobileSizing and 0.08 or 0.05
local updateSnowState
...
local function makeDraggable(object, dragObject, enableTaptic, tapticOffset)
	...
	local renderStepped = RunService.RenderStepped:Connect(function()
		if dragging and not Hidden then
			...
			local dynamicOffset = Main.AbsoluteSize.Y / 2 + 17
			if enableTaptic and tapticOffset then
				TweenService:Create(object, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Position = UDim2.fromOffset(position.X, position.Y)}):Play()
				TweenService:Create(dragObject.Parent, TweenInfo.new(0.05, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Position = UDim2.fromOffset(position.X, position.Y + dynamicOffset)}):Play()
			else
				if dragBar and tapticOffset then
					dragBar.Position = UDim2.fromOffset(position.X, position.Y + dynamicOffset)
				end
...
function RayfieldLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		local hasImage = data.Image ~= nil and data.Image ~= 0
		local titlePosition = newNotification.Title.Position
		local titleSize = newNotification.Title.Size
		local descriptionPosition = newNotification.Description.Position
		local descriptionSize = newNotification.Description.Size
		local textOffset = hasImage and 0 or -44

		local progressBar = Instance.new("Frame")
		progressBar.Name = "DurationBar"
		progressBar.AnchorPoint = Vector2.new(0, 1)
		progressBar.Position = UDim2.new(0, 20, 1, -8)
		progressBar.Size = UDim2.new(1, -40, 0, 2)
		progressBar.BackgroundColor3 = Color3.fromRGB(90, 95, 90)
		progressBar.BorderSizePixel = 0
		progressBar.BackgroundTransparency = 1
		progressBar.ZIndex = newNotification.ZIndex + 1
		progressBar.Parent = newNotification

		-- Set Data
		newNotification.Title.Text = data.Title or "Unknown Title"
		newNotification.Description.Text = data.Content or "Unknown Content"
		newNotification.Title.Position = UDim2.new(titlePosition.X.Scale, titlePosition.X.Offset + textOffset, titlePosition.Y.Scale, titlePosition.Y.Offset)
		newNotification.Title.Size = UDim2.new(titleSize.X.Scale, titleSize.X.Offset - textOffset, titleSize.Y.Scale, titleSize.Y.Offset)
		newNotification.Description.Position = UDim2.new(descriptionPosition.X.Scale, descriptionPosition.X.Offset + textOffset, descriptionPosition.Y.Scale, descriptionPosition.Y.Offset)
		newNotification.Description.Size = UDim2.new(descriptionSize.X.Scale, descriptionSize.X.Offset - textOffset, descriptionSize.Y.Scale, descriptionSize.Y.Offset)
		...
		TweenService:Create(newNotification, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Size = UDim2.new(1, 0, 0, math.max(bounds[1] + bounds[2] + 39, 68))}):Play()
		...
		TweenService:Create(progressBar, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0}):Play()
		...
		TweenService:Create(progressBar, TweenInfo.new(data.Duration or waitDuration, Enum.EasingStyle.Linear), {Size = UDim2.new(0, 0, 0, 2)}):Play()
		...
		TweenService:Create(progressBar, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {BackgroundTransparency = 1}):Play()
...
local function createSnowFlake()
	if not snowRunning or #Snowflakes >= maxSnow then
		return
	end
	...
	flake.Position = UDim2.fromOffset(math.random(0, math.max(Main.AbsoluteSize.X - size, 1)), -10)
	flake.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	flake.BackgroundTransparency = math.random(20, 40) / 100
	...
	local tween = TweenService:Create(flake, TweenInfo.new(math.random(2, 15), Enum.EasingStyle.Linear), {
		Position = UDim2.fromOffset(flake.Position.X.Offset, Main.AbsoluteSize.Y + 10)
	})
...
updateSnowState = function()
	local shouldSnow = snowEnabled and not Hidden and not Minimised and Main.Visible and not rayfieldDestroyed
	SnowContainer.Visible = shouldSnow
	if shouldSnow then
		if snowRunning then
			return
		end
		snowRunning = true
		task.spawn(function()
			while snowRunning and not rayfieldDestroyed do
				createSnowFlake()
				task.wait(snowSpawnDelay)
			end
		end)
	else
		snowRunning = false
...
local function updateSetting(category: string, setting: string, value: any)
	...
	if category == "System" and setting == "snow" then
		snowEnabled = value
		updateSnowState()
	end
	saveSettings()
...
local DKGC_BASE = "https://b6791824-1065-4839-b4ab-c6bad46ead19-00-2mcbfk9iw54r3.janeway.replit.dev/api"

local function gcHttpGet(url)
	if requestFunc then
...
local function gcHttpPost(url, body)
	if requestFunc then
		pcall(function() requestFunc({Url=url, Method="POST", Headers={["Content-Type"]="application/json"}, Body=body}) end)
	else
		pcall(function() HttpService:PostAsync(url, body, Enum.HttpContentType.ApplicationJson) end)
	end
end

local function gcHasLink(s)
	local lo = s:lower()
	for _, p in ipairs({"https?://","discord%.gg/","discord%.com/","www%.","%a+%.com[/%s%z]","%a+%.gg[/%s%z]"}) do
		if lo:match(p) then return true end
	end
	return false
end

local function createGlobalChat(window)
	window:CreateTab("Global Chat", 10734888000, true)
	if TabList["Global Chat"] then TabList["Global Chat"].Visible = true; TabList["Global Chat"].LayoutOrder = 999 end
	if Elements["Global Chat"] then Elements["Global Chat"].LayoutOrder = 999 end

	local gcPage = Elements:FindFirstChild("Global Chat")
	if not gcPage then return end

	local overlay = Instance.new("Frame", Main)
	overlay.Name = "GlobalChatOverlay"
	overlay.Position = useMobileSizing and UDim2.new(0, 12, 0, 92) or UDim2.new(0, 16, 0, 96)
	overlay.Size = useMobileSizing and UDim2.new(1, -24, 1, -104) or UDim2.new(1, -32, 1, -112)
	...
	local function connectHideBtn(btn)
		if btn.ClassName == "Frame" and btn.Name ~= "Global Chat" and btn.Name ~= "Template" and btn.Name ~= "Placeholder" then
			local interact = btn:FindFirstChild("Interact")
			if interact then interact.MouseButton1Click:Connect(function() overlay.Visible = false end) end
		end
	end
	for _, btn in ipairs(TabList:GetChildren()) do connectHideBtn(btn) end
	TabList.ChildAdded:Connect(connectHideBtn)

	local gcBtn = TabList["Global Chat"]
	if gcBtn and gcBtn:FindFirstChild("Interact") then
		gcBtn.Interact.MouseButton1Click:Connect(function() overlay.Visible = true end)
	end

	local gcShell = Instance.new("Frame", overlay)
	gcShell.Size = UDim2.new(1, 0, 1, 0)
	gcShell.BackgroundColor3 = Color3.fromRGB(14, 14, 14)
	gcShell.BorderSizePixel = 0
	gcShell.ZIndex = 10
	Instance.new("UICorner", gcShell).CornerRadius = UDim.new(0, 10)
	local gcShellStroke = Instance.new("UIStroke", gcShell)
	gcShellStroke.Color = Color3.fromRGB(34, 34, 34)
	gcShellStroke.Thickness = 1
	local gcActive = Instance.new("TextLabel", overlay)
	gcActive.AnchorPoint = Vector2.new(1, 0)
	gcActive.Position = UDim2.new(1, -12, 0, 18)
	gcActive.AutomaticSize = Enum.AutomaticSize.X
	gcActive.Size = UDim2.new(0, 0, 0, 22)
	gcActive.BackgroundTransparency = 1
	gcActive.BorderSizePixel = 0
	gcActive.ZIndex = 11
	gcActive.Font = Enum.Font.GothamMedium
	gcActive.Text = "0 online"
	gcActive.TextColor3 = Color3.fromRGB(60, 220, 100)
	gcActive.TextSize = 12
	local gcActivePad = Instance.new("UIPadding", gcActive)
	gcActivePad.PaddingLeft = UDim.new(0, 2)

	local gcScroll = Instance.new("ScrollingFrame", overlay)
	gcScroll.Size = UDim2.new(1, -12, 1, -92)
	gcScroll.Position = UDim2.new(0, 6, 0, 42)
	gcScroll.BackgroundTransparency = 1
	gcScroll.BorderSizePixel = 0
	gcScroll.ScrollBarThickness = 4
	gcScroll.ScrollBarImageColor3 = Color3.fromRGB(60,60,60)
	gcScroll.CanvasSize = UDim2.new(0,0,0,0)
	gcScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
	gcScroll.ZIndex = 11
	local gcLL = Instance.new("UIListLayout", gcScroll)
	gcLL.Padding = UDim.new(0,6); gcLL.SortOrder = Enum.SortOrder.LayoutOrder
	local gcPad = Instance.new("UIPadding", gcScroll)
	gcPad.PaddingTop = UDim.new(0,6); gcPad.PaddingBottom = UDim.new(0,6)
	gcPad.PaddingLeft = UDim.new(0,8); gcPad.PaddingRight = UDim.new(0,8)

	local gcInputBg = Instance.new("Frame", overlay)
	gcInputBg.Size = UDim2.new(1,-66,0,38)
	gcInputBg.Position = UDim2.new(0,6,1,-44)
	gcInputBg.BackgroundColor3 = Color3.fromRGB(18,18,18)
	gcInputBg.BorderSizePixel = 0
	gcInputBg.ZIndex = 11
	Instance.new("UICorner", gcInputBg).CornerRadius = UDim.new(0,10)
	local gcIStroke = Instance.new("UIStroke", gcInputBg)
	gcIStroke.Color = Color3.fromRGB(60,220,100); gcIStroke.Thickness = 1

	local gcMsgBox = Instance.new("TextBox", gcInputBg)
	gcMsgBox.Size = UDim2.new(1,-16,1,0); gcMsgBox.Position = UDim2.new(0,10,0,0)
	gcMsgBox.BackgroundTransparency = 1; gcMsgBox.PlaceholderText = "Send a message here..."
	gcMsgBox.PlaceholderColor3 = Color3.fromRGB(90,90,90); gcMsgBox.Text = ""
	gcMsgBox.TextColor3 = Color3.fromRGB(235,235,235); gcMsgBox.TextSize = 13
	gcMsgBox.Font = Enum.Font.Gotham; gcMsgBox.TextXAlignment = Enum.TextXAlignment.Left
	gcMsgBox.ClearTextOnFocus = false
	gcMsgBox.ZIndex = 12
	gcMsgBox.Focused:Connect(function() TweenService:Create(gcIStroke, TweenInfo.new(0.2), {Color=Color3.fromRGB(90,235,120)}):Play() end)
	gcMsgBox.FocusLost:Connect(function() TweenService:Create(gcIStroke, TweenInfo.new(0.2), {Color=Color3.fromRGB(60,220,100)}):Play() end)

	local gcArrow, gcArrowOffset, gcArrowSize = resolveIcon("send")
	local gcSendBtn = Instance.new("TextButton", overlay)
	gcSendBtn.Size = UDim2.new(0,44,0,38); gcSendBtn.Position = UDim2.new(1,-50,1,-44)
	gcSendBtn.BackgroundColor3 = Color3.fromRGB(60,220,100); gcSendBtn.Text = ""
	gcSendBtn.TextColor3 = Color3.fromRGB(255,255,255); gcSendBtn.TextSize = 13
	gcSendBtn.Font = Enum.Font.GothamBold
	gcSendBtn.BorderSizePixel = 0
	gcSendBtn.ZIndex = 11
	Instance.new("UICorner", gcSendBtn).CornerRadius = UDim.new(0,10)
	local gcSendIcon = Instance.new("ImageLabel", gcSendBtn)
	gcSendIcon.AnchorPoint = Vector2.new(0.5, 0.5)
	gcSendIcon.Position = UDim2.new(0.5, 0, 0.5, 0)
	gcSendIcon.Size = UDim2.new(0, 18, 0, 18)
	gcSendIcon.BackgroundTransparency = 1
	gcSendIcon.Image = gcArrow
	gcSendIcon.ImageColor3 = Color3.fromRGB(255,255,255)
	gcSendIcon.ZIndex = 12
	if gcArrowOffset then gcSendIcon.ImageRectOffset = gcArrowOffset end
	if gcArrowSize then gcSendIcon.ImageRectSize = gcArrowSize end
	gcSendBtn.MouseEnter:Connect(function() TweenService:Create(gcSendBtn, TweenInfo.new(0.15), {BackgroundColor3=Color3.fromRGB(90,235,120)}):Play() end)
	gcSendBtn.MouseLeave:Connect(function() TweenService:Create(gcSendBtn, TweenInfo.new(0.15), {BackgroundColor3=Color3.fromRGB(60,220,100)}):Play() end)

	local gcLocalPlayer = Players.LocalPlayer
	local gcMsgOrder = 0
	local gcThumbs = {}
	local gcUsers = {[gcLocalPlayer.Name] = gcLocalPlayer.UserId}
	local function gcThumbFor(username)
		local cached = gcThumbs[username]
		if cached ~= nil then return cached end
		local userId = gcUsers[username]
		if not userId then
			...
			gcUsers[username] = userId or false
		end
		if not userId then gcThumbs[username] = false; return false end
		local thumb = false
		pcall(function()
			thumb = Players:GetUserThumbnailAsync(userId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48)
		end)
		gcThumbs[username] = thumb or false
		return gcThumbs[username]
	end
	local function gcAddMsg(username, text)
		gcMsgOrder += 1
		local row = Instance.new("Frame", gcScroll)
		row.Size = UDim2.new(1,0,0,0); row.AutomaticSize = Enum.AutomaticSize.Y
		row.BackgroundColor3 = Color3.fromRGB(18,18,18); row.LayoutOrder = gcMsgOrder
		row.BorderSizePixel = 0; row.ZIndex = 11
		...
		avatar.ZIndex = 12
		local thumb = gcThumbFor(username)
		if thumb then avatar.Image = thumb end
		Instance.new("UICorner", avatar).CornerRadius = UDim.new(1, 0)
		local lbl = Instance.new("TextLabel", row)
		lbl.Size = UDim2.new(1,-47,0,0); lbl.Position = UDim2.new(0,40,0,7); lbl.AutomaticSize = Enum.AutomaticSize.Y
		lbl.BackgroundTransparency = 1; lbl.TextWrapped = true; lbl.RichText = true
		local nc = username == gcLocalPlayer.Name and "#ffffff" or "#b8b8b8"
		lbl.Text = string.format('<font color="%s"><b>%s</b></font>\n<font color="#d0d0d0">%s</font>', nc, username, text)
		lbl.TextSize = 13; lbl.Font = Enum.Font.Gotham; lbl.TextXAlignment = Enum.TextXAlignment.Left
		lbl.TextYAlignment = Enum.TextYAlignment.Top; lbl.ZIndex = 12
		row.AutomaticSize = Enum.AutomaticSize.Y
		task.defer(function() gcScroll.CanvasPosition = Vector2.new(0, gcScroll.AbsoluteCanvasSize.Y) end)
	end

	local function gcFlashError(msg)
		TweenService:Create(gcIStroke, TweenInfo.new(0.1), {Color=Color3.fromRGB(220,60,60)}):Play()
		gcMsgBox.PlaceholderText = msg; gcMsgBox.Text = ""
		task.delay(2, function()
			TweenService:Create(gcIStroke, TweenInfo.new(0.3), {Color=Color3.fromRGB(60,220,100)}):Play()
			gcMsgBox.PlaceholderText = "Send a message here..."
		end)
	end

	local gcLastSend = 0; local gcCooldown = false
	local function gcUpdateCooldown()
		if gcCooldown then return end
		local timeLeft = 1 - (tick() - gcLastSend)
		if timeLeft > 0 then
			gcCooldown = true
			gcSendBtn.Text = math.ceil(timeLeft)
			gcSendBtn.BackgroundColor3 = Color3.fromRGB(60,60,60)
			gcSendBtn.TextColor3 = Color3.fromRGB(150,150,150)
			task.spawn(function()
				while timeLeft > 0 do
					task.wait(0.1); timeLeft = 1 - (tick() - gcLastSend)
					if timeLeft > 0 then gcSendBtn.Text = math.ceil(timeLeft); gcSendIcon.Visible = false end
				end
						gcSendBtn.Text = ""
						gcSendIcon.Visible = true
					gcSendBtn.BackgroundColor3 = Color3.fromRGB(60,220,100)
				gcSendBtn.TextColor3 = Color3.fromRGB(255,255,255)
				gcCooldown = false
			end)
		end
	end

	local function gcSend()
		if gcCooldown then return end
		local text = gcMsgBox.Text
		if text == "" then return end
		if #text > 200 then gcFlashError("Too long! (max 200)"); return end
		if gcHasLink(text) then gcFlashError("Links aren't allowed!"); return end
		gcLastSend = tick(); gcUpdateCooldown()
		gcMsgBox.Text = ""; gcAddMsg(gcLocalPlayer.Name, text)
		task.spawn(function()
			gcHttpPost(DKGC_BASE.."/chat/messages", HttpService:JSONEncode({
				username = gcLocalPlayer.Name, message = text, gameId = tostring(game.PlaceId)
			}))
		end)
	end
	gcSendBtn.MouseButton1Click:Connect(gcSend)
	gcMsgBox.FocusLost:Connect(function(enter) if enter then gcSend() end end)

	local gcLastMsgId = 0
	task.spawn(function()
		local ok, msgs = pcall(function() return HttpService:JSONDecode(gcHttpGet(DKGC_BASE.."/chat/messages?limit=20")) end)
		if ok and type(msgs) == "table" then
			for _, m in ipairs(msgs) do
				if m.id and m.message then
					if m.id > gcLastMsgId then gcLastMsgId = m.id end
					gcAddMsg(m.username or "Unknown", m.message)
				end
			end
		end
	end)
	...
	task.spawn(function()
		while overlay.Parent do
			local ok, result = pcall(function() return HttpService:JSONDecode(gcHttpGet(DKGC_BASE.."/clients/online")) end)
			if ok and result and result.count ~= nil then
				gcActive.Text = tostring(result.count) .. " online"
			else
				gcActive.Text = "-- online"
			end
			task.wait(5)
		end
	end)
	...
	task.spawn(function()
		while overlay.Parent do
			task.wait(2)
			local ok, msgs = pcall(function() return HttpService:JSONDecode(gcHttpGet(DKGC_BASE.."/chat/messages?limit=50")) end)
			if ok and type(msgs) == "table" then
				for _, m in ipairs(msgs) do
					if m.id and m.id > gcLastMsgId then
						gcLastMsgId = m.id
						if m.username ~= gcLocalPlayer.Name then gcAddMsg(m.username, m.message) end
					end
				end
			end
		end
...
local function createSettings(window)
	...
	local configTab = window:CreateTab('Settings', 10723344270, true)

	if TabList['Settings'] then
		TabList['Settings'].Visible = true
		TabList['Settings'].LayoutOrder = 1001
	end

	if Elements['Settings'] then
		Elements['Settings'].LayoutOrder = 1001
	end

	do
		local selectedConfig = CFileName or ""
		local pendingName = ""

		local function getConfigs()
			local configs = {}
			if callSafely(isfolder, ConfigurationFolder) then
				for _, file in ipairs(callSafely(listfiles, ConfigurationFolder) or {}) do
					local name = file:match("([^/\\]+)$")
					if name and name:sub(-#ConfigurationExtension) == ConfigurationExtension then
						name = name:sub(1, -#ConfigurationExtension - 1)
		...
		configTab:CreateSection('Configurations')
		...
		local ConfigDropdown = configTab:CreateDropdown({
			...
			Callback = function(option)
				selectedConfig = type(option) == 'table' and option[1] or option
			end,
		})
		...
		configTab:CreateButton({
			Name = 'Save Config',
			Callback = function()
				local name = pendingName ~= '' and pendingName or selectedConfig
				if name ~= '' then
					ensureFolder(ConfigurationFolder)
					CFileName = name
					selectedConfig = name
					SaveConfiguration()
					RayfieldLibrary:Notify({Title = 'Config Saved', Content = "Saved as '" .. name .. "'", Duration = 3})
		...
		configTab:CreateButton({
			...
			Callback = function()
				if selectedConfig ~= '' then
					local path = ConfigurationFolder .. '/' .. selectedConfig .. ConfigurationExtension
					if callSafely(isfile, path) then
						CFileName = selectedConfig
						LoadConfiguration(callSafely(readfile, path))
						RayfieldLibrary:Notify({Title = 'Config Loaded', Content = "Loaded '" .. selectedConfig .. "'", Duration = 3})
					else
						RayfieldLibrary:Notify({Title = 'Not Found', Content = "Config '" .. selectedConfig .. "' does not exist.", Duration = 3})
					end
				end
			end,
		})
			...
			Callback = function()
				if selectedConfig ~= '' then
					...
					if callSafely(isfile, path) then
						...
						RayfieldLibrary:Notify({Title = 'Deleted', Content = "Deleted '" .. selectedConfig .. "'", Duration = 3})
						ConfigDropdown:Refresh(getConfigs())
						selectedConfig = ''
					else
						RayfieldLibrary:Notify({Title = 'Not Found', Content = "Config '" .. selectedConfig .. "' does not exist.", Duration = 3})
		...
		configTab:CreateSection('Extra')

		settingsTable.System.snow.Element = configTab:CreateToggle({
			Name = settingsTable.System.snow.Name,
			CurrentValue = settingsTable.System.snow.Value,
			Ext = true,
			Callback = function(Value)
				updateSetting('System', 'snow', Value)
			end,
		})

		settingsTable.General.rayfieldOpen.Element = configTab:CreateKeybind({
			Name = settingsTable.General.rayfieldOpen.Name,
			CurrentKeybind = settingsTable.General.rayfieldOpen.Value,
			HoldToInteract = false,
			Ext = true,
			CallOnChange = true,
			Callback = function(Value)
				updateSetting('General', 'rayfieldOpen', Value)
			end,
		})

		settingsTable.System.usageAnalytics.Element = configTab:CreateToggle({
			Name = settingsTable.System.usageAnalytics.Name,
			CurrentValue = settingsTable.System.usageAnalytics.Value,
			Ext = true,
			Callback = function(Value)
				updateSetting('System', 'usageAnalytics', Value)
			end,
		})
	end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	pcall(function()
		if not Settings.ConfigurationSaving.FileName or Settings.ConfigurationSaving.FileName == '' then
	...
	resizeHandle.MouseEnter:Connect(function()
		if resizing or Hidden then return end
		TweenService:Create(resizeHandle, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(1, 8, 1, 8), Position = UDim2.new(0, -4, 0, -4), ImageTransparency = 0.5}):Play()
	end)

	resizeHandle.MouseLeave:Connect(function()
		if resizing or Hidden then return end
		TweenService:Create(resizeHandle, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0, 0, 0, 0), ImageTransparency = 0.7}):Play()
	end)

	resizeHandle.InputBegan:Connect(function(input)
		if Minimised or Hidden then return end
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			resizing = true
			TweenService:Create(resizeHandle, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(1, 8, 1, 8), Position = UDim2.new(0, -4, 0, -4), ImageTransparency = 0}):Play()
	...
	UserInputService.InputChanged:Connect(function(input)
		...
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
			...
			local newW = math.max(minResizeW, sizeStart.X + delta.X)
			local newH = math.max(minResizeH, sizeStart.Y + delta.Y)
			Main.Size = UDim2.new(0, newW, 0, newH)
			Topbar.Size = UDim2.new(0, newW, 0, 45)
			if dragBar then dragBar.Position = UDim2.new(Main.Position.X.Scale, Main.Position.X.Offset, Main.Position.Y.Scale, Main.Position.Y.Offset + newH / 2 + 17) end
		end
	end)
...
function Tab:CreateToggle(ToggleSettings)
	...
	ColorPicker.BackgroundTransparency = 1 -- remove main background
	ColorPicker.UIStroke.Enabled = false -- remove green outline stroke
	...
	ColorPicker.Interact.ZIndex = 1 -- put this behind other elements
	Main.ImageTransparency = 1
	Background.BackgroundTransparency = 1
	
	-- remove old RGB/Hex
	if ColorPicker:FindFirstChild("RGB") then ColorPicker.RGB:Destroy() end
	if ColorPicker:FindFirstChild("HexInput") then ColorPicker.HexInput:Destroy() end
	
	-- define h,s,v FIRST before anything uses them
	...
	local RainbowToggle = Instance.new("Frame")
	RainbowToggle.Name = "RainbowToggle"
	RainbowToggle.Size = UDim2.new(0, 100, 0, 20)
	RainbowToggle.Position = UDim2.new(0, 17, 0, 50)
	RainbowToggle.BackgroundTransparency = 1
	RainbowToggle.ZIndex = 5 -- make sure it's above Interact
	...
	ToggleLabel.Parent = RainbowToggle
	
	local ToggleSwitch = Instance.new("Frame")
	ToggleSwitch.Name = "Switch"
	ToggleSwitch.Size = UDim2.new(0, 36, 0, 18)
	ToggleSwitch.Position = UDim2.new(1, -36, 0.5, -9)
	ToggleSwitch.BackgroundColor3 = SelectedTheme.ToggleDisabled
	ToggleSwitch.ZIndex = 6
	ToggleSwitch.Parent = RainbowToggle
...
ColorPicker.Interact.MouseButton1Down:Connect(function()
    if not opened then
        ...
        if useMobileSizing then
            -- MOBILE: Keep old position (centered), but smaller
            local posX = (screenSize.X - 200) / 2
            local posY = (screenSize.Y - 200) / 2
            
            ColorPickerFrame.Position = UDim2.fromOffset(posX, posY)
            ColorPickerFrame.Size = UDim2.new(0, 200, 0, 0) -- Smaller on mobile
            
            TweenService:Create(ColorPicker, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Size = UDim2.new(1, -10, 0, 75)}):Play()
            ...
            TweenService:Create(RainbowToggle, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Position = UDim2.new(0, 17, 0, 50)}):Play()
            TweenService:Create(ColorPicker.Interact, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Size = UDim2.new(0.574, 0, 1, 0)}):Play()
        else
            -- PC: Position to the SIDE (like in your image)
            local posX = cpPos.X + cpSize.X + 10  -- To the right of the ColorPicker
            local posY = cpPos.Y - 10  -- Slightly above
            
            -- If it would go off screen, position to the left instead
            if posX + 200 > screenSize.X then
                posX = cpPos.X - 210  -- Left side
            end
            
            -- Clamp to screen
            posX = math.max(10, math.min(posX, screenSize.X - 220))
            posY = math.max(10, math.min(posY, screenSize.Y - 220))
            
            ColorPickerFrame.Position = UDim2.fromOffset(posX, posY)
            ColorPickerFrame.Size = UDim2.new(0, 200, 0, 0) -- Smaller on PC too
            
            -- PC: Keep color picker compact
            TweenService:Create(ColorPicker, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Size = UDim2.new(1, -10, 0, 75)}):Play()
            TweenService:Create(Background, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, 173, 0, 86)}):Play()
            TweenService:Create(Display, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundTransparency = 1}):Play()
            TweenService:Create(ColorPicker.Interact, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Position = UDim2.new(0.289, 0, 0.5, 0)}):Play()
        ...
        ColorPickerFrame.Visible = true
        TweenService:Create(ColorPickerFrame, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, 200, 0, 210)}):Play() -- Smaller size (210 instead of 231)
        ...
        TweenService:Create(RainbowToggle, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Position = UDim2.new(0, 17, 0, 70)}):Play()
	...
	ToggleButton.Text = ""
	ToggleButton.ZIndex = 10 -- force on top
	ToggleButton.Active = true
	ToggleButton.Parent = RainbowToggle
	
	ToggleButton.MouseButton1Click:Connect(function()
		rainbowEnabled = not rainbowEnabled
		if rainbowEnabled then
			TweenService:Create(ToggleIndicator, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(1, -16, 0.5, -7)}):Play()
			TweenService:Create(ToggleSwitch, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ToggleEnabled}):Play()
			...
			rainbowConnection = RunService.RenderStepped:Connect(function()
				if rainbowEnabled then
					...
					setDisplay()
					local color = Color3.fromHSV(h,s,v)
					pcall(function() ColorPickerSettings.Callback(color) end)
					ColorPickerSettings.Color = color
				end
			end)
		else
			TweenService:Create(ToggleIndicator, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(0, 2, 0.5, -7)}):Play()
			TweenService:Create(ToggleSwitch, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ToggleDisabled}):Play()
	...
	Main.MouseButton1Down:Connect(function()
		if opened then
			mainDragging = true 
			rainbowEnabled = false -- stop rainbow when manually picking
			if rainbowConnection then rainbowConnection:Disconnect() rainbowConnection = nil end
			TweenService:Create(ToggleIndicator, TweenInfo.new(0.25, Enum.EasingStyle.Quart), {Position = UDim2.new(0, 2, 0.5, -7)}):Play()
			TweenService:Create(ToggleSwitch, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ToggleDisabled}):Play()
		end
	end)
	...
	function ColorPickerSettings:SetRainbow(state)
		rainbowEnabled = state
		if rainbowEnabled then
			TweenService:Create(ToggleIndicator, TweenInfo.new(0.25, Enum.EasingStyle.Quart), {Position = UDim2.new(1, -16, 0.5, -7)}):Play()
			TweenService:Create(ToggleSwitch, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ToggleEnabled}):Play()
	...
	Label.Title.Size = UDim2.new(1, -20, 0, 14)
	Label.Title.AutomaticSize = Enum.AutomaticSize.Y
	Label.AutomaticSize = Enum.AutomaticSize.Y
	Label.Visible = true
	Label.Parent = TabPage

	-- Force background and stroke to be invisible
	...
	-- Start text + icon transparent for tween-in
	Label.Icon.ImageTransparency = 1
	Label.Title.TextTransparency = 1

	-- Only tween text and icon, never background
	TweenService:Create(Label.Icon, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageTransparency = Icon and 0.2 or 1}):Play()
	...
	function LabelValue:Set(NewLabel, Icon, Color)
		...
		-- Keep background invisible on updates
		Label.BackgroundTransparency = 1
		Label.UIStroke.Transparency = 1
		...
		elseif Label:FindFirstChild('Icon') then
			Label.Icon.Image = ""
			Label.Icon.Visible = false
		end
	end

	-- Removed theme listener so it doesn't reset background
	-- If you want theme support for text color, add it back here but only for Label.Title.TextColor3
	...
	return LabelValue
		...
		function Tab:CreateParagraph(ParagraphSettings)
			...
			Paragraph.Title.TextWrapped = true
			Paragraph.Content.TextWrapped = true
			Paragraph.Title.Size = UDim2.new(1, -20, 0, 14)
			Paragraph.Content.Size = UDim2.new(1, -20, 0, 14)
			Paragraph.Title.AutomaticSize = Enum.AutomaticSize.Y
	...
	-- Make sure List has a UIListLayout
	...
	DropdownSearchIcon.Name = "Icon"
	DropdownSearchIcon.BackgroundTransparency = 1
	DropdownSearchIcon.Image = "rbxassetid://6031154871" -- default search icon
	DropdownSearchIcon.ImageColor3 = SelectedTheme.TextColor
	DropdownSearchIcon.Size = UDim2.new(0, 16, 0, 16)
	DropdownSearchIcon.Position = UDim2.new(0, 8, 0.5, -8)
	DropdownSearchIcon.ZIndex = 11
	DropdownSearchIcon.Parent = DropdownSearch

	local DropdownSearchInput = Instance.new("TextBox")
	DropdownSearchInput.BackgroundTransparency = 1
	DropdownSearchInput.BorderSizePixel = 0
	DropdownSearchInput.ClearTextOnFocus = false
	DropdownSearchInput.PlaceholderColor3 = SelectedTheme.TextColor
	DropdownSearchInput.PlaceholderText = "Search..."
	DropdownSearchInput.Position = UDim2.new(0, 32, 0, 0) -- shifted for icon
	DropdownSearchInput.Size = UDim2.new(1, -42, 1, 0) -- adjusted for icon
	DropdownSearchInput.Text = ""
	DropdownSearchInput.TextColor3 = SelectedTheme.TextColor
	DropdownSearchInput.Font = Dropdown.Selected.Font
	DropdownSearchInput.TextSize = Dropdown.Selected.TextSize
	DropdownSearchInput.TextXAlignment = Enum.TextXAlignment.Left
	DropdownSearchInput.TextYAlignment = Dropdown.Selected.TextYAlignment
	DropdownSearchInput.ZIndex = 11
	DropdownSearchInput.Parent = DropdownSearch

	-- Updated to ignore search bar
	local function IsDropdownOption(Option)
		return Option.ClassName == "Frame" and Option.Name ~= "Placeholder" and Option.Name ~= "DropdownSearchBar"
	end

	local function ApplyDropdownFilter()
		local searchText = string.lower(DropdownSearchInput.Text)
		for _, Option in ipairs(Dropdown.List:GetChildren()) do
			if IsDropdownOption(Option) then
				Option.Visible = searchText == "" or string.find(string.lower(Option.Name), searchText, 1, true) ~= nil
			end
		end
	end
	...
	Dropdown.Interact.MouseButton1Click:Connect(function()
		...
		if Dropdown.List.Visible then
			...
			for _, DropdownOpt in ipairs(Dropdown.List:GetChildren()) do
				if IsDropdownOption(DropdownOpt) then
			...
			TweenService:Create(Dropdown, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = UDim2.new(1, -10, 0, 222)}):Play()
		...
		for _, Option in ipairs(DropdownSettings.Options) do
			...
			DropdownOption.Interact.MouseButton1Click:Connect(function()
				...
				for _, droption in ipairs(Dropdown.List:GetChildren()) do
					if IsDropdownOption(droption) and not table.find(DropdownSettings.CurrentOption, droption.Name) then
	...
	for _, droption in ipairs(Dropdown.List:GetChildren()) do
		if IsDropdownOption(droption) then
	...
	DropdownSearchInput:GetPropertyChangedSignal("Text"):Connect(ApplyDropdownFilter)
	...
	Rayfield.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
		...
		DropdownSearchInput.PlaceholderColor3 = SelectedTheme.TextColor
		DropdownSearchInput.TextColor3 = SelectedTheme.TextColor
		DropdownSearchIcon.ImageColor3 = SelectedTheme.TextColor -- update icon color
		TweenService:Create(Dropdown, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
	end)
	...
	local success, result = pcall(function()
		pcall(createGlobalChat, Window)
		createSettings(Window)
	end)

	if not success then warn('Rayfield had an issue creating settings.') end
	snowEnabled = getSetting("System", "snow")
	updateSnowState()

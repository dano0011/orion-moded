...
do
    ...
    local githubBase =
        "https://raw.githubusercontent.com/dano0011/12/refs/heads/main/ids/"
...
local Library do
    ...
    do
        local Success, Result = pcall(function()
            return loadstring(game:HttpGet(
                "https://raw.githubusercontent.com/dano0011/12/refs/heads/main/lucide",
                true
            ))()
        end)
        ...
        if Success and type(Result) == "table" then
            ...
            warn("Failed to load Lucide:", Result)
        end
    end
    ...
    local KeyValidationScriptId = "d8824b23a4d9f2e0d62b4e69397d206b"
    local KeyValidationInterval = 30
    local KeyValidationStopped = false
    ...
    Library = {
        ...
        GlobalChat = {
                HttpUrl = "http://13.140.160.87:3000",
                WsUrl = "ws://13.140.160.87:3000/ws",
            Token = "aaa717d44364f124a267b6d595316734f0f1d483aec6ab5449fd33811a80d4ca",
            PollInterval = 3,
            MessageLimit = 50,
            PresenceInterval = 5,
            RetryDelay = 5,
    ...
    Library.ValidateKey = function(self)
        if getgenv().ExpressHubKeyValidated ~= true then
            return true
        end

        local Validator = getgenv().ValidateKey
        if type(Validator) == "function" then
            local Success, Result = pcall(Validator)
            return Success and Result or nil
        end

        local Key = tostring(getgenv().script_key or "")
        if Key == "" then
            return nil
        end
        ...
        if not Success or type(Api) ~= "table" or type(Api.check_key) ~= "function" then
            return nil
        end

        Api.script_id = KeyValidationScriptId
        local CheckSuccess, Status = pcall(function()
            return Api.check_key(Key)
        end)
        if not CheckSuccess or type(Status) ~= "table" then
            return nil
        end
    ...
    Library.StartKeyValidation = function(self)
        if getgenv().ExpressHubKeyValidated ~= true or self._KeyValidationStarted then
            return
        end

        self._KeyValidationStarted = true
        task.spawn(function()
            while Library and not KeyValidationStopped do
                if Library and not KeyValidationStopped then
                    local Valid = Library:ValidateKey()
                    if Valid == false then
                        pcall(function()
                            LocalPlayer:Kick("Your Express Hub key has expired or is no longer active.")
                        end)
                        break
                    end
                end

                task.wait(KeyValidationInterval)
            end
        end)
    end
    ...
    Library.SetToggleStyle = function(self, Style)
        ...
        if self.ToggleStyle == NewStyle then
            return
        end

        self.ToggleStyle = NewStyle

        for _, Callback in self.ToggleStyleCallbacks do
            Callback(NewStyle)
        end
    end
    ...
    Library.SetToggleSide = function(self, Side)
        ...
        if self.ToggleSide == NewSide then
            return
        end
    ...
    local function AttachAutoGlobalChat(Window)
        ...
        local SuggestionSubTab = GlobalChatTab:AddSubTab({
            Name = Window.GlobalChatOptions.SuggestionSubTabName or "Suggestions",
            Columns = 1
        })

        local ChatSystem = ChatSubTab:GlobalChat()
        ChatSystem:SetConnectedText(Window.GlobalChatOptions.InitialStatusText)
        local SuggestionSystem = SuggestionSubTab:GlobalChatSuggestions({
            ChatPage = ChatSubTab,
            ChatSystem = ChatSystem,
            Config = Window.GlobalChatOptions
        })
    ...
    local WelcomeSession = game.JobId
    local ShowWelcomeIntro = getgenv().ExpressHubWelcomeSessionV2 ~= WelcomeSession
    ...
    task.defer(function()
        for Index, Value in Library.Images do 
            ...
            local ImagePath = Library.Folders.Assets .. "/" .. ImageName
            
            if not isfile(ImagePath) then
                EnsureFile(ImagePath, ImageLink)
            end
        end
    end)
    ...
    local Instances = { } do
        ...
        Instances.Tooltip = function(self, Text)
            ...
            Library:Connect(Gui.MouseEnter, function()
                if Library.HideTooltips or Library.ActiveBlockedSubTab then return end
        ...
        Instances.MakeDraggable = function(self, Target)
            ...
            local IsWithin = function(Position)
                ...
                return Position.X >= AbsolutePosition.X and Position.X <= AbsolutePosition.X + AbsoluteSize.X
                and Position.Y >= AbsolutePosition.Y and Position.Y <= AbsolutePosition.Y + AbsoluteSize.Y
            end
            ...
            Library:Connect(UserInputService.InputBegan, function(Input)
                if IsWithin(Input.Position) then
                    BeginDrag(Input)
                end
            end)
        ...
        Instances.MakeResizeable = function(self, Minimum, Maximum)
            ...
            local ResizeButton = Instances:Create("ImageButton", {
				Parent = Gui,
                Image = "rbxassetid://106214494928059",
            ...
            local ResizeBaseSize = UDim2New(0, 48, 0, 48)
            local ResizeHoverSize = UDim2New(0, 51, 0, 51)
            local ResizePressSize = UDim2New(0, 54, 0, 54)

            ResizeButton:Connect("MouseEnter", function()
                ResizeButton:Tween(nil, {Size = ResizeHoverSize})
            end)

            ResizeButton:Connect("MouseLeave", function()
                ResizeButton:Tween(nil, {Size = ResizeBaseSize})
            end)

            ResizeButton:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    Resizing = true
                    ResizeButton:Tween(nil, {Size = ResizePressSize})
                    ...
                    InputChanged = Input.Changed:Connect(function()
                        if Input.UserInputState == Enum.UserInputState.End then
                            Resizing = false
                            ResizeButton:Tween(nil, {Size = Library:IsMouseOverFrame(ResizeButton) and ResizeHoverSize or ResizeBaseSize})
        ...
        Instances.OnHover = function(self, Function)
            ...
            return Library:Connect(self.Instance.MouseEnter, function(...)
                if Library.ActiveBlockedSubTab then
                    return
                end
    ...
    local CustomFont = { } do
        function CustomFont:New(Name, Weight, Style, Data)
            if not EnsureFile(Data.Id, Data.Url) then
    ...
    Library.GetImage = function(self, Image)
        ...
        local ImagePath = self.Folders.Assets .. "/" .. ImageData[1]
        if not isfile(ImagePath) then
            EnsureFile(ImagePath, ImageData[2])
        end
    ...
    Library.ResolveElementFlag = function(self, Flag, Name)
        if type(Flag) == "string" and StringGSub(Flag, "%s+", "") ~= "" then
            return Flag
        end
        ...
        if type(Name) == "string" and StringGSub(Name, "%s+", "") ~= "" then
            if self.Elements[Name] == nil and self.SetFlags[Name] == nil then
                return Name
            end
        end
    ...
    Library.LoadConfig = function(self, Config)
        ...
        local NormalizeConfigValue = function(Flag, Value)
            ...
            if type(Value) == "table" then
                ...
                if Value.Color then
                    ...
                    if Value.Alpha ~= nil and type(Value.Alpha) ~= "number" then
                        Value.Alpha = tonumber(Value.Alpha)
                    end

                    if Value.Alpha == nil or type(Value.Alpha) == "number" then
                        return Value
                    end
        ...
        if FailedLoads > 0 then
            return true, "Some settings did not load because this config is outdated"
        end
    ...
    local function CloneConfigValue(Value)
        ...
        for Index, Item in next, Value do
            NewValue[Index] = CloneConfigValue(Item)
        end
    ...
    local function CompareConfigValues(Left, Right)
        ...
        for Index, Value in next, Left do
            if not CompareConfigValues(Value, Right[Index]) then
                return false
            end
        end

        for Index in next, Right do
    ...
    Library.WriteAutoSaveConfig = function(self)
        ...
        if Config == self._LastAutoSaveConfig and isfile(AutoSaveConfigFile) then
            return true
        end
    ...
    Library.QueueAutoSave = function(self)
        if not self.AutoSaveReady or self.AutoSaveApplying or not self:IsAutoSaveEnabled() then
            return
        end

        self._AutoSaveQueuedAt = OsClock()

        if self._AutoSaveQueued then
            return
        end
        ...
        task.spawn(function()
            while Library and Library._AutoSaveQueued do
                if OsClock() - (Library._AutoSaveQueuedAt or 0) >= 0.35 then
                    break
                end
            ...
            if not Library.AutoSaveReady or Library.AutoSaveApplying or not Library:IsAutoSaveEnabled() then
                return
            end

            pcall(function()
                Library:WriteAutoSaveConfig()
            end)
        end)
    end

    Library.LoadAutoSaveConfig = function(self)
        if not self:IsAutoSaveEnabled() or not isfile(AutoSaveConfigFile) then
            return false
        end

        self.AutoSaveApplying = true

        local Success, LoadSuccess, Error = pcall(function()
            return self:LoadConfig(readfile(AutoSaveConfigFile))
        end)

        self.AutoSaveApplying = false

        if not Success then
            return false, LoadSuccess
        end

        if LoadSuccess then
            self._LastAutoSaveConfig = readfile(AutoSaveConfigFile)
        end
    ...
    Library.IsMouseOverFrame = function(self, Frame)
        ...
        return MousePosition.X >= AbsolutePosition.X and MousePosition.X <= AbsolutePosition.X + AbsoluteSize.X 
        and MousePosition.Y >= AbsolutePosition.Y and MousePosition.Y <= AbsolutePosition.Y + AbsoluteSize.Y
    end
    ...
    Library.ApplyPageSearch = function(self, Page, Query)
        ...
        for _, SearchData in SearchEntries do
            ...
            local Matches = not HasQuery
                or StringFind(SearchName, NormalizedQuery, 1, true) ~= nil
                or (SectionName ~= "" and StringFind(SectionName, NormalizedQuery, 1, true) ~= nil)

            if SearchData.Item and SearchData.Item.Instance then
                SearchData.Item.Instance.Visible = Matches
            end

            if Matches and Section then
    ...
    Library.CreateColorpicker = function(self, Data)
        ...
        local Items = { } do
            ...
            Items["Indicator"] = Instances:Create("Frame", {
                ...
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(0, Library.ToggleStyle == "Checkbox" and 18 or 32, 0, 18),
                BorderSizePixel = 0,
            ...
            Items["Text"] = Instances:Create("TextLabel", {
                ...
                BorderSizePixel = 0,
                BackgroundTransparency = 1,
                Position = Library.ToggleSide == "Right" and UDim2New(0, 0, 0.5, 0) or (Library.ToggleStyle == "Checkbox" and UDim2New(0, 26, 0.5, 0) or UDim2New(0, 40, 0.5, 0)),
                BorderColor3 = FromRGB(0, 0, 0),
                AutomaticSize = Enum.AutomaticSize.X,
                TextSize = 14,
                BackgroundColor3 = FromRGB(255, 255, 255)
        ...
        local IsRainbowHovered = false
        local UpdateRainbowVisuals
        ...
        local function EnsureSlideConnection()
            ...
            SlideInputConnection = Library:Connect(UserInputService.InputChanged, function(Input)
                ...
                if not SlidingPalette and not SlidingHue then
                    DisconnectSlideConnection()
                end
            end, SlideConnectionName)
        end

        UpdateRainbowVisuals = function()
            ...
            local IndicatorTheme = IsRainbowToggled and "Accent" or (IsRainbowHovered and "Hovered Element" or "Element")
            ...
            Items["Text"]:Tween(nil, {TextTransparency = IsRainbowToggled and 0 or (IsRainbowHovered and 0.15 or 0.4)})

            if IsCheckbox then
                Items["Knob"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
                    Position = UDim2New(0, 3, 0.5, 0),
                    Size = UDim2New(0, 0, 0, 0)
                })
                Items["Check"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                    ImageTransparency = IsRainbowToggled and 0 or 1,
                    Size = IsRainbowToggled and UDim2New(0, 8, 0, 8) or UDim2New(0, 0, 0, 0)
                })
            else
                Items["Check"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                    ImageTransparency = 1,
                ...
                Items["Knob"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
                    Position = IsRainbowToggled and UDim2New(0, 17, 0.5, 0) or UDim2New(0, 3, 0.5, 0),
                    Size = UDim2New(0, 12, 0, 12)
                })
            end
        end

        Items["RainbowToggle"]:OnHover(function()
            IsRainbowHovered = true
            UpdateRainbowVisuals()
        end)

        Items["RainbowToggle"]:OnHoverLeave(function()
            IsRainbowHovered = false
            UpdateRainbowVisuals()
        end)
        ...
        Library:RegisterToggleStyleCallback(function()
            UpdateRainbowVisuals()
        end)
        ...
        Items["RgbInput"]:Connect("Focused", function()
            ...
            local function ApplyRgb()
                ...
                Red = MathClamp(Red, 0, 255)
                Green = MathClamp(Green, 0, 255)
                Blue = MathClamp(Blue, 0, 255)
    ...
    Library.CreateKeybind = function(self, Data)
        ...
        function Keybind:Set(Key)
            if StringFind(tostring(Key), "Enum") then 
                self.Key = Key.Name == "Backspace" and "None" or tostring(Key)
            ...
            elseif type(Key) == "table" then
                ...
                local IsNone = Key.Key == "Backspace" or Key.Key == "None" or RawKey == "Enum.KeyCode.Backspace" or RawKey == "None"
                local RealKey = IsNone and "None" or Key.Key
                self.Key = IsNone and "None" or RawKey
                ...
                local KeyString = IsNone and "None" or Keys[self.Key] or StringGSub(tostring(RealKey), "Enum.", "") or RealKey
    ...
    Library.ConnectGlobalChat = function(self)
        if not WebSocketFunction then
            return
        end

        if not self.GlobalChat or not self.GlobalChat.AutoConnect then
            return
        end
        ...
        local function HandlePacket(Packet)
            ...
            if type(Packet.hwidBan) == "table" and Packet.hwidBan.banned then
                ...
                task.defer(function()
                    pcall(function()
                        LocalPlayer:Kick(tostring(Packet.hwidBan.message or "Banned From ExpressHub"))
                    end)
                end)
                return
            end
            if PacketType == "hwid_ban" and Packet.banned then
                ...
                if TargetSenderId == "" or TargetSenderId == LocalSenderId then
                    ...
                    task.defer(function()
                        pcall(function()
                            LocalPlayer:Kick(tostring(Packet.message or "Banned From ExpressHub"))
                        end)
                    end)
                end
                return
            ...
            if PacketType == "force_update" then
                ...
                task.spawn(function()
                    ...
                    for Remaining = DelaySeconds - 1, 1, -1 do
                        ...
                        if self.ActiveGlobalAnnouncement == Announcement
            ...
            if PacketType == "announcement" and type(Packet.announcement) == "table" then
                ...
                self:MakeGlobalAnnouncement({
                    Name = Ann.title or "ExpressHub",
                    Content = Ann.content or "",
                    Time = Ann.duration or 7
                })
                return
            end
            ...
            if PacketType == "poll_start" and type(Packet.poll) == "table" then
                if self._ActivePoll then
                    pcall(function()
                        if self._ActivePoll.ClosePoll then
                            self._ActivePoll.ClosePoll()
                        end
                    end)
                ...
                local function MakeVoteButton(BtnIndex, BtnText)
                    ...
                    Btn.Instance.MouseButton1Click:Connect(function()
                        ...
                        if not VoteBody then
                            self._ActivePoll.HasVoted = false
                            return
                        end
                        ...
                        if not Ok then
                            if self._ActivePoll and self._ActivePoll.PollId == PollId then
                                self._ActivePoll.HasVoted = false
                            end
                            return
                        end
        ...
        local function Connect()
            ...
            local Success, Ws = pcall(function()
                return WebSocketFunction(BuildWsUrl())
            end)
            ...
            if MessageConnection and type(MessageConnection.Disconnect) == "function" then
                pcall(function()
                    MessageConnection:Disconnect()
                end)
            end

            if CloseConnection and type(CloseConnection.Disconnect) == "function" then
                pcall(function()
        ...
        TableInsert(self.CleanupCallbacks, function()
            ...
            Connected = false
            if PingThread then
                pcall(function() task.cancel(PingThread) end)
            end
            if MessageConnection and type(MessageConnection.Disconnect) == "function" then
                pcall(function()
                    MessageConnection:Disconnect()
    ...
    Library.AttachExpressButton = function(self, Window)
        ...
        local function CreateRipple()
            ...
            local RippleTween = TweenService:Create(Ripple.Instance, TweenInfo.new(0.5), {
        ...
        local function AnimateButton(IsOpen)
            TweenService:Create(Widgets.FrameScale.Instance, TweenInfo.new(0.08, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
                Scale = 0.88
            }):Play()

            TweenService:Create(Widgets.ImageStroke.Instance, TweenInfo.new(0.12, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
                Thickness = IsOpen and 2 or 1.25
            }):Play()

            task.delay(0.08, function()
                if not Widgets.MainFrame or not Widgets.MainFrame.Instance or not Widgets.MainFrame.Instance.Parent then
                    return
                end

                TweenService:Create(Widgets.FrameScale.Instance, TweenInfo.new(0.16, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                    Scale = 1
                }):Play()
            end)
        end
        ...
        local function CheckSettingsChanged()
            ...
            if Widgets.LastSettings.Toggle ~= Current.Toggle then
                if Widgets.MainGui then
                    Widgets.MainGui.Instance.Enabled = Current.Toggle
                end
                Widgets.LastSettings.Toggle = Current.Toggle
            end

            if Widgets.LastSettings.ShowButton ~= Current.ShowButton then
                if Widgets.MainFrame then
                    Widgets.MainFrame.Instance.Visible = Current.ShowButton
                end
        ...
        Widgets.FrameScale = Instances:Create("UIScale", {
            Parent = Widgets.MainFrame.Instance
        })

        Instances:Create("UICorner", {
            Parent = Widgets.MainFrame.Instance,
            CornerRadius = UDimNew(0, 25)
        })
        ...
        task.defer(function()
            if self._ExpressWidgets ~= Widgets then
                return
            end
    ...
    do -- Components
        Library.MakeWindow = function(self, Data)
            ...
            local Window = {
                ...
                GlobalChatOptions = {
                    ...
                    SuggestionSubTabName = Data.GlobalChatSuggestionSubTabName or Library.GlobalChat.SuggestionSubTabName or "Suggestions",
                    InitialStatusText = Data.GlobalChatInitialStatusText or Library.GlobalChat.InitialStatusText or "Connecting..."
                }
            }

            local MainFrameFinalSize = IsMobile and UDim2New(0, 575, 0, 345) or UDim2New(0, 730, 0, 530)
            local MainFrameCenteredPosition = UDim2New(0.5, 0, 0.5, 0)
            local IntroStartScale = IsMobile and 0.78 or 0.68
            ...
            local Items = { } do
                Items["MainFrame"] = Instances:Create("Frame", {
                    ...
                    Name = "\0",
                    AnchorPoint = Vector2New(0.5, 0.5),
                    Position = MainFrameCenteredPosition,
                    BorderColor3 = FromRGB(0, 0, 0),
                    Size = MainFrameFinalSize,
                    ClipsDescendants = true,
                    Visible = false,
                    BorderSizePixel = 0,
                    BackgroundColor3 = FromRGB(17, 16, 20)
                ...
                Items["IntroScale"] = Instances:Create("UIScale", {
                ...
                Items["GlowStroke"] = Instances:Create("UIStroke", {
                    Parent = Items["MainFrame"].Instance,
                    Name = "\0",
                    Thickness = 1,
                    Transparency = 0.72,
                    ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                    Color = Library.Theme.Accent
                })  Items["GlowStroke"]:AddToTheme({Color = "Accent"})

                Instances:Create("UIGradient", {
                    Parent = Items["GlowStroke"].Instance,
                    Name = "\0",
                    Rotation = 90,
                    Transparency = NumSequence{NumSequenceKeypoint(0, 0.92), NumSequenceKeypoint(0.2, 0.88), NumSequenceKeypoint(0.55, 0.72), NumSequenceKeypoint(1, 0.42)}
                })

                Items["GlowStrokeSoft"] = Instances:Create("UIStroke", {
                    Parent = Items["MainFrame"].Instance,
                    Name = "\0",
                    Thickness = 2,
                    Transparency = 0.9,
                    ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                    Color = Library.Theme.Accent
                })  Items["GlowStrokeSoft"]:AddToTheme({Color = "Accent"})

                Instances:Create("UIGradient", {
                    Parent = Items["GlowStrokeSoft"].Instance,
                    Name = "\0",
                    Rotation = 90,
                    Transparency = NumSequence{NumSequenceKeypoint(0, 0.98), NumSequenceKeypoint(0.2, 0.96), NumSequenceKeypoint(0.55, 0.9), NumSequenceKeypoint(1, 0.72)}
                })
                ...
                local ResizeHandle = Instances:Create("Frame", {
                    Parent = Library.Holder.Instance,
                    Name = "\0",
                    Size = UDim2New(0, 0, 0, 0),
                    Visible = false,
                ...
                Window.ResizeButton:OnHover(function()
                    Items["GlowStroke"]:Tween(nil, {Transparency = 0.58, Thickness = 1.2})
                    Items["GlowStrokeSoft"]:Tween(nil, {Transparency = 0.82, Thickness = 2.4})
                end)

                Window.ResizeButton:OnHoverLeave(function()
                    Items["GlowStroke"]:Tween(nil, {Transparency = 0.72, Thickness = 1})
                    Items["GlowStrokeSoft"]:Tween(nil, {Transparency = 0.9, Thickness = 2})
                end)
            ...
            function Window:SetOpen(Bool)
                ...
                if not Bool and Window.ClosePromptOpen then
                    SetClosePromptOpen(false)
                end
            ...
            local function ClosePlayerTools()
                if Window.PlayerToolClosers then
                    for _, CloseTools in Window.PlayerToolClosers do
                        CloseTools()
                    end
                end
            end
            ...
            function Window:SetStreamerMode(Bool)
                ...
                if Items["AccountAvatar"] then
                    Items["AccountAvatar"].Instance.Image = Avatar
                end

                if Items["AccountDisplayName"] then
                    Items["AccountDisplayName"].Instance.Text = DisplayName
                end

                if Items["AccountUsername"] then
                    Items["AccountUsername"].Instance.Text = "@" .. Username
                end
            end
            ...
            local function BindClosePromptButtonAnimation(Button, BaseSize, HoverSize, PressSize, BaseTransparency, HoverTransparency)
                ...
                Button:Connect("InputEnded", function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        Button:Tween(nil, {Size = Library:IsMouseOverFrame(Button) and HoverSize or BaseSize, TextTransparency = Library:IsMouseOverFrame(Button) and HoverTransparency or BaseTransparency})
                    end
                end)
            end
                ...
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    if Window.IsMinimized then
                        Library:MakeNotification({
                            Name = "UI Minimized",
                            Content = "Unminimize the UI to close the script.",
                            Time = 3
                        })
                        return
                    end
            ...
            Items["ClosePromptCancel"]:Connect("MouseButton1Click", function()
                SetClosePromptOpen(false)
            end)

            Items["ClosePromptConfirm"]:Connect("MouseButton1Click", function()
                CloseWindow()
            end)

            BindClosePromptButtonAnimation(
                Items["ClosePromptCancel"],
                UDim2New(0.5, -21, 0, 34),
                UDim2New(0.5, -18, 0, 36),
                UDim2New(0.5, -16, 0, 32),
                0,
                0.08
            )

            BindClosePromptButtonAnimation(
                Items["ClosePromptConfirm"],
                UDim2New(0.5, -21, 0, 34),
                UDim2New(0.5, -18, 0, 36),
                UDim2New(0.5, -16, 0, 32),
                0,
            ...
            Items["FullScreenButton"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    if IsFullScreen then
                        ...
                        Items["MainFrame"]:Tween(nil, {AnchorPoint = Vector2New(0, 0), Position = UDim2New(0, 0, 0, 0)})
                    else
                        Items["MainFrame"]:Tween(nil, {Size = UDim2New(0, OldSize.X, 0, OldSize.Y)})
                        Items["MainFrame"]:Tween(nil, {AnchorPoint = Vector2New(0.5, 0.5), Position = MainFrameCenteredPosition})
                    end
                end
            end)
            Window:SetOpen(true)

            if ShowWelcomeIntro then
                ShowWelcomeIntro = false

                local Intro = InstanceNew("Frame")
                Intro.Parent = Items["MainFrame"].Instance
                Intro.Name = "\0"
                Intro.Size = UDim2FromScale(1, 1)
                Intro.Active = true
                Intro.BorderSizePixel = 0
                Intro.ZIndex = 100
                Intro.BackgroundColor3 = FromRGB(18, 15, 25)
                Intro.BackgroundTransparency = 0.02
                InstanceNew("UICorner", Intro).CornerRadius = UDimNew(0, 6)

                local IntroGradient = InstanceNew("UIGradient")
                IntroGradient.Parent = Intro
                IntroGradient.Rotation = 35
                IntroGradient.Color = RGBSequence({
                    RGBSequenceKeypoint(0, FromRGB(28, 23, 40)),
                    RGBSequenceKeypoint(1, FromRGB(11, 10, 16))
                })

                local IntroStroke = InstanceNew("UIStroke")
                IntroStroke.Parent = Intro
                IntroStroke.Color = Library.Theme.Accent
                IntroStroke.Thickness = 1
                IntroStroke.Transparency = 0.5
                IntroStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

                local AvatarHalo = InstanceNew("Frame")
                AvatarHalo.Parent = Intro
                AvatarHalo.AnchorPoint = Vector2New(0.5, 0.5)
                AvatarHalo.Position = UDim2New(0.5, 0, IsMobile and 0.32 or 0.33, 0)
                AvatarHalo.Size = UDim2New(0, 0, 0, 0)
                AvatarHalo.BackgroundColor3 = Library.Theme.Accent
                AvatarHalo.BackgroundTransparency = 1
                AvatarHalo.BorderSizePixel = 0
                AvatarHalo.ZIndex = 101
                InstanceNew("UICorner", AvatarHalo).CornerRadius = UDimNew(1, 0)

                local HaloScale = InstanceNew("UIScale")
                HaloScale.Parent = AvatarHalo

                local Avatar = InstanceNew("ImageLabel")
                Avatar.Parent = Intro
                Avatar.AnchorPoint = Vector2New(0.5, 0.5)
                Avatar.Position = UDim2New(0.5, 0, IsMobile and 0.32 or 0.33, 0)
                Avatar.Size = UDim2New(0, 0, 0, 0)
                Avatar.BackgroundTransparency = 1
                Avatar.Image = Content
                Avatar.ImageTransparency = 1
                Avatar.ScaleType = Enum.ScaleType.Crop
                Avatar.ZIndex = 102
                InstanceNew("UICorner", Avatar).CornerRadius = UDimNew(1, 0)
                ...
                Title.Parent = Intro
                Title.AnchorPoint = Vector2New(0.5, 0)
                Title.Position = UDim2New(0.5, 0, IsMobile and 0.485 or 0.48, -2)
                Title.Size = UDim2New(0, IsMobile and 360 or 430, 0, IsMobile and 34 or 40)
                Title.BackgroundTransparency = 1
                Title.FontFace = Library.Font
                Title.Text = "Welcome, " .. LocalPlayer.Name
                Title.TextColor3 = FromRGB(255, 255, 255)
                Title.TextSize = IsMobile and 25 or 31
                Title.TextTransparency = 1
                Title.ZIndex = 102

                local Subtitle = InstanceNew("TextLabel")
                Subtitle.Parent = Intro
                Subtitle.AnchorPoint = Vector2New(0.5, 0)
                Subtitle.Position = UDim2New(0.5, 0, IsMobile and 0.56 or 0.555, 0)
                Subtitle.Size = UDim2New(0, IsMobile and 360 or 430, 0, IsMobile and 24 or 28)
                Subtitle.BackgroundTransparency = 1
                Subtitle.FontFace = Library.Subfont or Library.Font
                Subtitle.Text = "Preparing your experience"
                Subtitle.TextColor3 = FromRGB(255, 255, 255)
                Subtitle.TextSize = IsMobile and 14 or 16
                Subtitle.TextTransparency = 1
                Subtitle.ZIndex = 102

                local BarBack = InstanceNew("Frame")
                BarBack.Parent = Intro
                BarBack.AnchorPoint = Vector2New(0.5, 0.5)
                BarBack.Position = UDim2New(0.5, 0, IsMobile and 0.67 or 0.665, 0)
                BarBack.Size = UDim2New(0, IsMobile and 300 or 390, 0, IsMobile and 7 or 9)
                BarBack.BackgroundColor3 = FromRGB(255, 255, 255)
                BarBack.BackgroundTransparency = 1
                BarBack.BorderSizePixel = 0
                BarBack.ZIndex = 102
                InstanceNew("UICorner", BarBack).CornerRadius = UDimNew(1, 0)

                local Bar = InstanceNew("Frame")
                Bar.Parent = BarBack
                Bar.Size = UDim2New(0, 0, 1, 0)
                Bar.BackgroundColor3 = FromRGB(255, 255, 255)
                Bar.BackgroundTransparency = 0
                Bar.BorderSizePixel = 0
                Bar.ZIndex = 103
                InstanceNew("UICorner", Bar).CornerRadius = UDimNew(1, 0)

                local BarGradient = InstanceNew("UIGradient")
                BarGradient.Parent = Bar
                BarGradient.Color = RGBSequence({
                    RGBSequenceKeypoint(0, Library.Theme.Accent),
                    RGBSequenceKeypoint(0.5, FromRGB(255, 255, 255)),
                    RGBSequenceKeypoint(1, Library.Theme.Accent)
                })

                local Status = InstanceNew("TextLabel")
                Status.Parent = Intro
                Status.AnchorPoint = Vector2New(0.5, 0)
                Status.Position = UDim2New(0.5, 0, IsMobile and 0.705 or 0.7, 0)
                Status.Size = UDim2New(0, IsMobile and 360 or 430, 0, IsMobile and 22 or 26)
                Status.BackgroundTransparency = 1
                Status.FontFace = Library.Subfont or Library.Font
                Status.Text = "Loading interface"
                Status.TextColor3 = FromRGB(255, 255, 255)
                Status.TextSize = IsMobile and 13 or 15
                Status.TextTransparency = 1
                Status.ZIndex = 102

                local HaloPulse = TweenService:Create(HaloScale, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {Scale = 1.08})

                TweenService:Create(AvatarHalo, TweenInfo.new(0.65, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2New(0, IsMobile and 142 or 172, 0, IsMobile and 142 or 172), BackgroundTransparency = 0.84}):Play()
                TweenService:Create(Avatar, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2New(0, IsMobile and 108 or 132, 0, IsMobile and 108 or 132), ImageTransparency = 0}):Play()
                TweenService:Create(Title, TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {TextTransparency = 0, Position = UDim2New(0.5, 0, IsMobile and 0.485 or 0.48, -12)}):Play()
                TweenService:Create(Subtitle, TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {TextTransparency = 0.35, Position = UDim2New(0.5, 0, IsMobile and 0.56 or 0.555, -8)}):Play()
                TweenService:Create(BarBack, TweenInfo.new(0.45, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {BackgroundTransparency = 0.84}):Play()
                TweenService:Create(Status, TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {TextTransparency = 0.45, Position = UDim2New(0.5, 0, IsMobile and 0.705 or 0.7, -8)}):Play()
                TweenService:Create(Bar, TweenInfo.new(7.4, Enum.EasingStyle.Quart, Enum.EasingDirection.InOut), {Size = UDim2New(1, 0, 1, 0)}):Play()
                HaloPulse:Play()
                getgenv().ExpressHubWelcomeSessionV2 = WelcomeSession
                task.spawn(function()
                    task.wait(7.55)
                    ...
                    TweenService:Create(
                        Items["IntroScale"].Instance,
                        TweenInfo.new(0.7, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                        {Scale = 1}
                    ):Play()
                    TweenService:Create(Intro, TweenInfo.new(0.55, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {BackgroundTransparency = 0.18}):Play()
                    TweenService:Create(IntroStroke, TweenInfo.new(0.55, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Transparency = 0.18}):Play()
                    TweenService:Create(AvatarHalo, TweenInfo.new(0.45, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {BackgroundTransparency = 0.9}):Play()
                    TweenService:Create(BarBack, TweenInfo.new(0.45, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {BackgroundTransparency = 0.92}):Play()
                    ...
                    HaloPulse:Cancel()
                    TweenService:Create(Intro, TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {BackgroundTransparency = 1}):Play()
                    TweenService:Create(IntroStroke, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Transparency = 1}):Play()
                    TweenService:Create(AvatarHalo, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Size = UDim2New(0, 126, 0, 126), BackgroundTransparency = 1}):Play()
                    TweenService:Create(Avatar, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Size = UDim2New(0, 88, 0, 88), ImageTransparency = 1}):Play()
                    TweenService:Create(AvatarStroke, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Transparency = 1}):Play()
                    TweenService:Create(Title, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {TextTransparency = 1}):Play()
                    TweenService:Create(Subtitle, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {TextTransparency = 1}):Play()
                    TweenService:Create(BarBack, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {BackgroundTransparency = 1}):Play()
                    TweenService:Create(Bar, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {BackgroundTransparency = 1}):Play()
                    TweenService:Create(Status, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {TextTransparency = 1}):Play()
                ...
                Items["IntroScale"].Instance.Scale = 1
            end

            task.defer(function()
                Library:AttachExpressButton(Window)
            end)
            ...
            if Window.GlobalChatOptions.Enabled and not Window.AutoGlobalChat then
                task.defer(function()
                    AttachAutoGlobalChat(result)
                end)
            end
            ...
            if not IsMobile and Library.GlobalChat and Library.GlobalChat.AutoConnect and not Library._GlobalChatConnected then
                ...
                task.defer(function()
                    local Success = pcall(function()
                        Library:ConnectGlobalChat()
                    end)

                    if not Success then
                        Library._GlobalChatConnected = false
                    end
                end)
            end
        ...
        Library.MakeTab = function(self, Data)
            ...
            if type(Page.Icon) == "string" and Page.Icon ~= "" then
                if Page.Icon:find("rbxassetid://", 1, true)
                    or Page.Icon:find("rbxasset://", 1, true)
                    or Page.Icon:find("http", 1, true) then
                ...
                elseif Lucide then
                    local Success, Result = pcall(function()
                        return Lucide.GetAsset(Page.Icon, 48)
                    end)

                    if Success and Result then
                        TabIconData = Result
                    else
                        warn("Invalid Lucide tab icon:", Page.Icon, Result)
                    end
                end
            end
            ...
            Items["Text"] = Instances:Create("TextLabel", {
                ...
                BorderSizePixel = 0,
                BackgroundTransparency = 1,
                Position = UDim2New(0, TabIconData and 34 or 10, 0.5, 0),
                BorderColor3 = FromRGB(0, 0, 0),
                AutomaticSize = Enum.AutomaticSize.X,
                TextSize = 18,
                BackgroundColor3 = FromRGB(255, 255, 255)
            ...
            if TabIconData then
                Items["Icon"] = Instances:Create("ImageLabel", {
                    ...
                    BorderColor3 = FromRGB(0, 0, 0),
                    AnchorPoint = Vector2New(0, 0.5),
                    Image = TabIconData.Url,
                    ImageRectSize = TabIconData.ImageRectSize,
                    ImageRectOffset = TabIconData.ImageRectOffset,
                    BackgroundTransparency = 1,
            ...
            function Page:Turn(Bool)
                ...
                if Page.Active then
                    Items["Inactive"]:Tween(nil, {BackgroundTransparency = 0.91})
        ...
        Library.Pages.AddSubTab = function(self, Data)
            ...
            local Page = {
                ...
                Blocked = Data.Blocked == true or Data.blocked == true or type(Data.Blocked) == "string" or type(Data.blocked) == "string",
                BlockedText = (type(Data.Blocked) == "string" and Data.Blocked) or (type(Data.blocked) == "string" and Data.blocked) or Data.BlockedText or Data.blockedText or "Temporarily Disabled",
            ...
            local Items = { } do
                ...
                Items["BlockedOverlay"] = Instances:Create("TextButton", {
                    Parent = Items["Page"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    Text = "",
                    ...
                    ZIndex = 50,
                    TextSize = 14
                })  Items["BlockedOverlay"]:AddToTheme({BackgroundColor3 = "Background"})

                Instances:Create("UICorner", {
                    Parent = Items["BlockedOverlay"].Instance,
                    Name = "\0"
                })

                Items["BlockedText"] = Instances:Create("TextLabel", {
                    Parent = Items["BlockedOverlay"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    TextColor3 = FromRGB(255, 255, 255),
                    BorderColor3 = FromRGB(0, 0, 0),
                    Text = Page.BlockedText,
                    BorderSizePixel = 0,
                    BackgroundTransparency = 1,
                    AnchorPoint = Vector2New(0.5, 0.5),
                    Position = UDim2New(0.5, 0, 0.5, 0),
                    Size = UDim2New(1, -24, 0, 60),
                    ZIndex = 51,
                    TextWrapped = true,
                    TextXAlignment = Enum.TextXAlignment.Center,
                    ...
                    TextSize = 26,
                    BackgroundColor3 = FromRGB(255, 255, 255)
                })  Items["BlockedText"]:AddToTheme({TextColor3 = "Text"})
            ...
            function Page:Turn(Bool)
                ...
                if Bool then
                    Library.ActiveBlockedSubTab = Page.Blocked and Page or nil
                    Items["Text"]:Tween(nil, {TextTransparency = 0})
                    ...
                    if Page.Blocked then
                        Items["BlockedOverlay"].Instance.Visible = true
                        Items["BlockedText"].Instance.Text = Page.BlockedText
                        for _, Value in Page.Sections do
                            Value:SetOpen(true)
                        end
                    else
                        Items["BlockedOverlay"].Instance.Visible = false
                    end
                    ...
                    if Library.ActiveBlockedSubTab == Page then
                        Library.ActiveBlockedSubTab = nil
                    end
                    Items["Text"]:Tween(nil, {TextTransparency = 0.4})
                    Items["Inactive"]:Tween(nil, {BackgroundTransparency = 1})
            ...
            Items["Inactive"]:Connect("InputBegan", function(Input)
                ...
                if Page.Blocked then
                    Items["BlockedOverlay"].Instance.Visible = Page.Active
                    Items["BlockedText"].Instance.Text = Page.BlockedText
                end
        ...
        Library.Pages.Playerlist = function(self, Data)
            ...
            local Items = { } do
                ...
                Items["CloseTools"] = Instances:Create("TextButton", {
                    Parent = Items["SettingsWindow"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    TextColor3 = FromRGB(255, 255, 255),
                    ...
                    AutoButtonColor = false,
                    BackgroundColor3 = FromRGB(41, 40, 49)
                })  Items["CloseTools"]:AddToTheme({BackgroundColor3 = "Element", TextColor3 = "Text"})

                Instances:Create("UICorner", {
                    Parent = Items["CloseTools"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 5)
                })
            ...
            local ToolsPlayerDropdown

            local function GetPlayerNames()
                local Names = { }
                for _, Player in Players:GetPlayers() do
                    TableInsert(Names, Player.Name)
                end
                return Names
            end

            local function UpdateToolsPlayerDropdown()
                if not ToolsPlayerDropdown then
                    return
                end

                local ShowDropdown = IsSettingsOpen and not Playerlist.Page.Page.Active and not Playerlist.Window.IsMinimized
                if ToolsPlayerDropdown.IsOpen and not ShowDropdown then
                    ToolsPlayerDropdown:SetOpen(false)
                end

                ToolsPlayerDropdown:SetVisibility(ShowDropdown)
            end

            local function CloseTools()
                if Toggle.Value then
                    Toggle:Set(false)
                elseif RenderStepped2 then
                    RenderStepped2:Disconnect()
                    RenderStepped2 = nil
                end
            end

            Playerlist.Window.PlayerToolClosers = Playerlist.Window.PlayerToolClosers or { }
            TableInsert(Playerlist.Window.PlayerToolClosers, CloseTools)

            ToolsPlayerDropdown = Playerlist:AddDropdown({
                Name = "Player",
                Default = "",
                Flag = "PlayerToolsPlayer",
            ...
            UpdateToolsPlayerDropdown()

            Items["CloseTools"]:Connect("MouseButton1Click", CloseTools)
            Library:Connect(Playerlist.Page.Page.Items["Page"].Instance:GetPropertyChangedSignal("Visible"), UpdateToolsPlayerDropdown)

            Toggle.Callback = function(Value)
                IsSettingsOpen = Value

                if IsSettingsOpen and Playerlist.Window.IsMinimized then
                    Toggle:Set(false)
                    return
                end
                ...
                if IsSettingsOpen then 
                    ...
                    if ToolsPlayerDropdown and ToolsPlayerDropdown.IsOpen then
                        ToolsPlayerDropdown:SetOpen(false)
                    end
            ...
            function Playerlist:Remove(Name)
                if Playerlist.Players[Name] then
                    if Playerlist.Player == Playerlist.Players[Name].Player then
            ...
            Library:Thread(function()
                ...
                Library:Connect(RunService.Heartbeat, function()
                    ...
                    if not Playerlist.Page.Active then
                        return
                    end

                    for _, Value in Playerlist.Players do
...
Library.Pages.GlobalChat = function(self, Data)
            ...
            local function RebuildFilterPatterns()
                ...
                for _, Word in BackendState.FilterWords do
                    if type(Word) == "string" and Word ~= "" then
                        ...
                        for Index = 1, #Word do
                            local Original = Word:sub(Index, Index)
                            if Original:match("%a") then
                                Parts[#Parts + 1] = "[" .. Original:upper() .. Original:lower() .. "]"
                            else
                                Parts[#Parts + 1] = Original:gsub("[%(%)%.%%%+%-%*%?%[%^%$]", "%%%1")
                            end
                        end

                        BackendState.FilterPatterns[#BackendState.FilterPatterns + 1] = "(" .. table.concat(Parts, "[^%a%d]*") .. ")"
                    end
                end
            end
            ...
            local function FilterMessageText(Text, SenderId)
                ...
                if #BackendState.FilterWords == 0 then return Text end

                local CharMap = {}
                for i = 1, #Text do
                    local c = StringSub(Text, i, i)
                    if c:match("[%a%d]") then
                        CharMap[#CharMap + 1] = { ch = c:lower(), pos = i }
                    end
                end

                local NormParts = table.create(#CharMap)
                for i, Entry in CharMap do
                    NormParts[i] = Entry.ch
                end
                local NormText = table.concat(NormParts)

                local ResultBytes = {}
                for i = 1, #Text do
                    ResultBytes[i] = StringSub(Text, i, i)
                end

                for _, Word in BackendState.FilterWords do
                    if type(Word) ~= "string" or Word == "" then continue end
                    local NormWord = Word:lower():gsub("[^%a%d]", "")
                    if NormWord == "" then continue end
                    local WLen = #NormWord
                    local TLen = #NormText
                    local i = 1
                    while i <= TLen - WLen + 1 do
                        if StringSub(NormText, i, i + WLen - 1) == NormWord then
            ...
            Library:Connect(RunService.RenderStepped, function()
                ...
                RefreshBottomDistance()
                if Btn.Visible and IsMessagesCloseEnoughToDismiss() then
                    DismissJumpBanner()
                end
            end)
...
Library.Pages.GlobalChatSuggestions = function(self, Data)
            Data = Data or { }

            local SuggestionSystem = {
                Window = self.Window,
                Page = self,
            }

            if SuggestionSystem.Page.Columns ~= 1 then
                error("GlobalChatSuggestions can only be in one column (Edit Columns property to 1)")
            end

            local SuggestionConfig = NormalizeGlobalChatConfig(Data.Config or Library.GlobalChat)
            local Draft = {
                GameTitle = "",
                Content = "",
                Type = "",
            ...
            local SuggestionDestroyed = false
            local SuccessPayload = nil
            local MailboxOpen = false
            local ReviewUnreadCount = 0
            local KnownSuggestionStatuses = { }
            local SeenSuggestionStatuses = { }
            local PendingCancelSuggestion = nil
            local DefaultMailIcon = "rbxassetid://136371270317895"
            local ReviewMailIcon = "rbxassetid://93740348829800"
            local SuggestionSeenCacheFile = Library.Folders.Cache .. "/suggestion_seen_" .. tostring(LocalPlayer.UserId) .. ".json"

            do
                local OldPath = Library.Folders.Configs .. "/suggestion_seen_" .. tostring(LocalPlayer.UserId) .. ".json"
                if isfile(OldPath) then
                    if not isfile(SuggestionSeenCacheFile) then
                        pcall(function() writefile(SuggestionSeenCacheFile, readfile(OldPath)) end)
                    end
                    pcall(function() delfile(OldPath) end)
                end

                if isfile(SuggestionSeenCacheFile) then
                    local Cached = JsonDecode(readfile(SuggestionSeenCacheFile))
                    if type(Cached) == "table" then
                        for EntryId, EntryStatus in Cached do
                            local NumericId = tonumber(EntryId)
                            if NumericId then
                                SeenSuggestionStatuses[NumericId] = tostring(EntryStatus or "under_review")
                            end
                        end
                    end
                end
            ...
            local function GetSuggestionHeaders(Config)
                local Headers = {
                    ["Accept"] = "application/json",
                }
            ...
            local function BuildSuggestionAuthorizedUrl(Config, BaseUrl, Query)
                local FinalQuery = type(Query) == "table" and TableClone(Query) or { }
            ...
            local function GetResolvedGameName(Callback)
                return ResolvePlaceName(Callback)
            end
            ...
            local function GetSuggestionTypeLabel(Value)
                if Value == "patched_broken_feature" then
                    return "Patched/Broken Feature"
            ...
            local function GetStatusColor(Value)
                if Value == "approved" then
                    return FromRGB(90, 214, 128)
                elseif Value == "rejected" then
                    return FromRGB(255, 102, 102)
                end

                return FromRGB(255, 209, 102)
            end

            local function GetStatusIcon(Value)
                if Value == "approved" then
                    return "rbxassetid://136434898133814"
                elseif Value == "rejected" then
                    return "rbxassetid://88572064423642"
                end
            ...
            local function TruncatePreview(Text, Limit)
                ...
                Clean = StringGSub(Clean, "%s+", " ")
                if StringLen(Clean) <= Limit then
                    return Clean
                end

                return StringSub(Clean, 1, math.max(1, Limit - 3)) .. "..."
            end

            local function ClearSuggestionRows()
                for _, Entry in SuggestionRows do
            ...
            local function SetOverlayVisible(Overlay, Panel, Bool)
                Overlay.Instance.Visible = Bool
                Panel.Instance.Visible = Bool

                if Bool then
                    Overlay.Instance.BackgroundTransparency = 1
                    Panel.Instance.BackgroundTransparency = 1
                    Panel.Instance.Position = UDim2New(0.5, 0, 0.5, 20)
                    Overlay:Tween(TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0.22})
                    Panel:Tween(TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                        BackgroundTransparency = 0,
                        Position = UDim2New(0.5, 0, 0.5, 0)
                    })
                else
            ...
            local Items = { } do
                Items["Root"] = Instances:Create("Frame", {
                    Parent = SuggestionSystem.Page.ColumnsData[1].Instance,
                    Name = "\0",
                    BorderColor3 = FromRGB(0, 0, 0),
                    Size = UDim2New(1, 0, 1, 0),
                    BorderSizePixel = 0,
                    BackgroundColor3 = FromRGB(26, 25, 31)
                })  Items["Root"]:AddToTheme({BackgroundColor3 = "Inline"})
                ...
                Items["HeaderTitle"] = Instances:Create("TextLabel", {
                    ...
                    FontFace = Library.Font,
                    TextColor3 = FromRGB(255, 255, 255),
                    Text = "Suggestion Submission",
                    Position = UDim2New(0, 14, 0, 14),
                    Size = UDim2New(1, -70, 0, 24),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    TextSize = 16,
                    TextXAlignment = Enum.TextXAlignment.Left
                ...
                Items["HeaderHint"] = Instances:Create("TextLabel", {
                    Parent = Items["Root"].Instance,
                    Name = "\0",
                    FontFace = Library.Subfont,
                    TextColor3 = FromRGB(180, 180, 190),
                    Text = "Send a suggestion or bug report straight to staff review.",
                    Position = UDim2New(0, 14, 0, 38),
                    Size = UDim2New(1, -90, 0, 18),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    TextSize = 12,
                    TextXAlignment = Enum.TextXAlignment.Left
                })  Items["HeaderHint"]:AddToTheme({TextColor3 = "Text"})

                Items["MailButton"] = Instances:Create("ImageButton", {
                    Parent = Items["Root"].Instance,
                    Name = "\0",
                    AnchorPoint = Vector2New(1, 0),
                    ...
                    AutoButtonColor = false,
                    Image = ""
                })  Items["MailButton"]:AddToTheme({BackgroundColor3 = "Element"})

                Instances:Create("UICorner", {
                    Parent = Items["MailButton"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 6)
                })
                ...
                Items["MailButtonIcon"] = Instances:Create("ImageLabel", {
                    Parent = Items["MailButton"].Instance,
                    Name = "\0",
                    AnchorPoint = Vector2New(0.5, 0.5),
                    Position = UDim2New(0.5, 0, 0.5, 0),
                    ...
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    Image = DefaultMailIcon,
                    ImageColor3 = FromRGB(255, 255, 255),
                    ScaleType = Enum.ScaleType.Fit
                })

                Items["MailBadge"] = Instances:Create("Frame", {
                    Parent = Items["MailButton"].Instance,
                    Name = "\0",
                    AnchorPoint = Vector2New(1, 0),
                    Position = UDim2New(1, 4, 0, -4),
                ...
                Instances:Create("UICorner", {
                    Parent = Items["MailBadge"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(1, 0)
                })

                Items["MailBadgeText"] = Instances:Create("TextLabel", {
                    Parent = Items["MailBadge"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    TextColor3 = FromRGB(255, 255, 255),
                ...
                Items["FormScroller"] = Instances:Create("ScrollingFrame", {
                    Parent = Items["Root"].Instance,
                    Name = "\0",
                    Active = true,
                    AutomaticCanvasSize = Enum.AutomaticSize.Y,
                    ...
                    ScrollBarThickness = 2,
                    BackgroundTransparency = 1,
                    Position = UDim2New(0, 12, 0, 66),
                    Size = UDim2New(1, -24, 1, -140)
                })  Items["FormScroller"]:AddToTheme({ScrollBarImageColor3 = "Accent"})

                Instances:Create("UIPadding", {
                    Parent = Items["FormScroller"].Instance,
                    Name = "\0",
                    PaddingBottom = UDimNew(0, 6),
                    PaddingRight = UDimNew(0, 4)
                })
                ...
                local function MakeLabel(Text)
                    return Instances:Create("TextLabel", {
                        ...
                        TextSize = 13,
                        TextXAlignment = Enum.TextXAlignment.Left,
                        LayoutOrder = #Items["FormScroller"].Instance:GetChildren() + 1
                    })
                end

                Items["GameLabel"] = MakeLabel("Game Title")  Items["GameLabel"].Instance.LayoutOrder = 1  Items["GameLabel"]:AddToTheme({TextColor3 = "Text"})

                Items["GameTitleBg"] = Instances:Create("Frame", {
                    Parent = Items["FormScroller"].Instance,
                    Name = "\0",
                    Size = UDim2New(1, -2, 0, 46),
                    BorderSizePixel = 0,
                    BackgroundColor3 = FromRGB(41, 40, 49),
                    LayoutOrder = 2
                })  Items["GameTitleBg"]:AddToTheme({BackgroundColor3 = "Element"})

                Instances:Create("UICorner", {
                    Parent = Items["GameTitleBg"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 6)
                })

                Items["GameTitleInput"] = Instances:Create("TextBox", {
                    Parent = Items["GameTitleBg"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    ...
                    TextXAlignment = Enum.TextXAlignment.Left,
                    TextSize = 14
                })  Items["GameTitleInput"]:AddToTheme({TextColor3 = "Text"})

                Items["GameHint"] = Instances:Create("TextLabel", {
                    Parent = Items["FormScroller"].Instance,
                    Name = "\0",
                    FontFace = Library.Subfont,
                    TextColor3 = FromRGB(170, 170, 185),
                    Text = "Auto-filled from your current game. Replace it if you are suggesting something for a different game.",
                    Size = UDim2New(1, -2, 0, 28),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    TextWrapped = true,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    ...
                    TextSize = 12,
                    LayoutOrder = 3
                })  Items["GameHint"]:AddToTheme({TextColor3 = "Text"})

                Items["SuggestionLabel"] = MakeLabel("Suggestion Details")  Items["SuggestionLabel"].Instance.LayoutOrder = 4  Items["SuggestionLabel"]:AddToTheme({TextColor3 = "Text"})

                Items["SuggestionBg"] = Instances:Create("Frame", {
                    Parent = Items["FormScroller"].Instance,
                    Name = "\0",
                    Size = UDim2New(1, -2, 0, 190),
                    BorderSizePixel = 0,
                    BackgroundColor3 = FromRGB(41, 40, 49),
                    LayoutOrder = 5
                })  Items["SuggestionBg"]:AddToTheme({BackgroundColor3 = "Element"})

                Instances:Create("UICorner", {
                    Parent = Items["SuggestionBg"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 6)
                })

                Items["SuggestionInputScroll"] = Instances:Create("ScrollingFrame", {
                    Parent = Items["SuggestionBg"].Instance,
                    Name = "\0",
                    Active = true,
                    ...
                    Size = UDim2New(1, -24, 1, -42),
                    CanvasPosition = Vector2New(0, 0)
                })  Items["SuggestionInputScroll"]:AddToTheme({ScrollBarImageColor3 = "Accent"})

                Instances:Create("UIPadding", {
                    Parent = Items["SuggestionInputScroll"].Instance,
                    Name = "\0",
                    PaddingRight = UDimNew(0, 4)
                })

                Items["SuggestionInput"] = Instances:Create("TextBox", {
                    Parent = Items["SuggestionInputScroll"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    PlaceholderText = "Type here your suggestion feature or something that needs to be fixed or updated.",
                    PlaceholderColor3 = FromRGB(175, 175, 175),
                    Text = "",
                    MultiLine = true,
                    ClearTextOnFocus = false,
                    ...
                    BorderSizePixel = 0,
                    Position = UDim2New(0, 0, 0, 0),
                    Size = UDim2New(1, -6, 0, 138),
                    AutomaticSize = Enum.AutomaticSize.Y,
                    TextSize = 14
                })  Items["SuggestionInput"]:AddToTheme({TextColor3 = "Text"})

                Items["CounterLabel"] = Instances:Create("TextLabel", {
                    Parent = Items["SuggestionBg"].Instance,
                    Name = "\0",
                    FontFace = Library.Subfont,
                    TextColor3 = FromRGB(170, 170, 185),
                ...
                Items["TypeLabel"] = MakeLabel("What are you suggesting for?")  Items["TypeLabel"].Instance.LayoutOrder = 6  Items["TypeLabel"]:AddToTheme({TextColor3 = "Text"})

                Items["TypeHolder"] = Instances:Create("Frame", {
                    Parent = Items["FormScroller"].Instance,
                    Name = "\0",
                    Size = UDim2New(1, -2, 0, 118),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    LayoutOrder = 7
                })
                ...
                Instances:Create("UIListLayout", {
                    Parent = Items["TypeHolder"].Instance,
                    Name = "\0",
                    Padding = UDimNew(0, 8),
                    SortOrder = Enum.SortOrder.LayoutOrder
                })

                local function MakeTypeButton(Key, Label)
                    ...
                    local DotBg = Instances:Create("Frame", {
                        Parent = Button.Instance,
                        Name = "\0",
                        Position = UDim2New(0, 10, 0.5, -7),
                        Size = UDim2New(0, 14, 0, 14),
                        BorderSizePixel = 0,
                ...
                SuggestionItems["TypeButtons"] = {
                    MakeTypeButton("suggestion", "Suggestion"),
                    MakeTypeButton("patched_broken_feature", "Patched/Broken Feature"),
                    MakeTypeButton("both", "Both"),
                }

                Items["ActionBar"] = Instances:Create("Frame", {
                    Parent = Items["Root"].Instance,
                    Name = "\0",
                    AnchorPoint = Vector2New(0, 1),
                    Position = UDim2New(0, 12, 1, -12),
                    Size = UDim2New(1, -24, 0, 46),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                })

                local function MakeActionButton(Name, Color, Position, AnchorPoint)
                    local Button = Instances:Create("TextButton", {
                        Parent = Items["ActionBar"].Instance,
                        Name = "\0",
                        FontFace = Library.Font,
                        TextColor3 = FromRGB(255, 255, 255),
                        Text = Name,
                        AnchorPoint = AnchorPoint or Vector2New(0, 0),
                        Position = Position,
                        Size = UDim2New(0, 122, 0, 42),
                ...
                Items["CancelButton"] = MakeActionButton("Cancel", FromRGB(166, 74, 74), UDim2New(0, 0, 0, 0), Vector2New(0, 0))
                Items["SubmitButton"] = MakeActionButton("Submit", FromRGB(72, 176, 108), UDim2New(1, 0, 0, 0), Vector2New(1, 0))

                Items["MailButton"]:OnHover(function()
                    Items["MailButton"]:Tween(TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        BackgroundColor3 = Library.Theme["Hovered Element"]
                    })
                    Items["MailButtonIcon"]:Tween(TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        Size = UDim2New(0, 35, 0, 35)
                    })
                end)

                Items["MailButton"]:OnHoverLeave(function()
                    Items["MailButton"]:Tween(TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                ...
                Items["CancelButton"]:OnHover(function()
                    Items["CancelButton"]:Tween(TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        BackgroundColor3 = FromRGB(188, 86, 86),
                        Size = UDim2New(0, 126, 0, 42)
                    })
                end)

                Items["CancelButton"]:OnHoverLeave(function()
                    Items["CancelButton"]:Tween(TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        BackgroundColor3 = FromRGB(166, 74, 74),
                        Size = UDim2New(0, 122, 0, 42)
                    })
                end)

                Items["SubmitButton"]:OnHover(function()
                    Items["SubmitButton"]:Tween(TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        BackgroundColor3 = FromRGB(84, 194, 121),
                        Size = UDim2New(0, 126, 0, 42)
                    })
                end)

                Items["SubmitButton"]:OnHoverLeave(function()
                    Items["SubmitButton"]:Tween(TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        BackgroundColor3 = FromRGB(72, 176, 108),
                        Size = UDim2New(0, 122, 0, 42)
                    })
                end)

                Items["MailboxOverlay"] = Instances:Create("Frame", {
                    Parent = Items["Root"].Instance,
                    Name = "\0",
                    Size = UDim2New(1, 0, 1, 0),
                    BackgroundColor3 = FromRGB(8, 8, 10),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    Visible = false,
                    ZIndex = 60
                ...
                Items["MailboxPanel"] = Instances:Create("Frame", {
                    Parent = Items["MailboxOverlay"].Instance,
                    Name = "\0",
                    AnchorPoint = Vector2New(0.5, 0.5),
                    Position = UDim2New(0.5, 0, 0.5, 0),
                    Size = UDim2New(1, -48, 1, -56),
                    BackgroundColor3 = FromRGB(26, 25, 31),
                    BorderSizePixel = 0,
                    Visible = false,
                    ZIndex = 61
                })  Items["MailboxPanel"]:AddToTheme({BackgroundColor3 = "Inline"})

                Instances:Create("UICorner", {
                    Parent = Items["MailboxPanel"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 8)
                })

                Items["MailboxTitle"] = Instances:Create("TextLabel", {
                    Parent = Items["MailboxPanel"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    TextColor3 = FromRGB(255, 255, 255),
                    Text = "Submitted Suggestions",
                    Position = UDim2New(0, 14, 0, 14),
                    Size = UDim2New(1, -52, 0, 20),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    TextSize = 15,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    ZIndex = 62
                })  Items["MailboxTitle"]:AddToTheme({TextColor3 = "Text"})

                Items["MailboxClose"] = Instances:Create("TextButton", {
                    Parent = Items["MailboxPanel"].Instance,
                    Name = "\0",
                    Text = "X",
                    FontFace = Library.Font,
                    ...
                    Position = UDim2New(1, -38, 0, 10),
                    ZIndex = 62,
                    AutoButtonColor = false,
                    TextSize = 13
                })  Items["MailboxClose"]:AddToTheme({BackgroundColor3 = "Element", TextColor3 = "Text"})

                Instances:Create("UICorner", {
                    Parent = Items["MailboxClose"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 6)
                })

                Items["MailboxEmpty"] = Instances:Create("TextLabel", {
                    Parent = Items["MailboxPanel"].Instance,
                    Name = "\0",
                    FontFace = Library.Subfont,
                    TextColor3 = FromRGB(185, 185, 195),
                ...
                Items["MailboxScroller"] = Instances:Create("ScrollingFrame", {
                    Parent = Items["MailboxPanel"].Instance,
                    Name = "\0",
                    Active = true,
                    AutomaticCanvasSize = Enum.AutomaticSize.Y,
                    ...
                    Size = UDim2New(1, -24, 1, -64),
                    ZIndex = 62
                })  Items["MailboxScroller"]:AddToTheme({ScrollBarImageColor3 = "Accent"})

                Instances:Create("UIListLayout", {
                    Parent = Items["MailboxScroller"].Instance,
                    Name = "\0",
                    Padding = UDimNew(0, 10),
                    SortOrder = Enum.SortOrder.LayoutOrder
                })
                ...
                Items["SuccessOverlay"] = Instances:Create("Frame", {
                    Parent = Items["Root"].Instance,
                    Name = "\0",
                    Size = UDim2New(1, 0, 1, 0),
                    BackgroundColor3 = FromRGB(8, 8, 10),
                ...
                Items["SuccessPanel"] = Instances:Create("Frame", {
                    Parent = Items["SuccessOverlay"].Instance,
                    Name = "\0",
                    AnchorPoint = Vector2New(0.5, 0.5),
                    Position = UDim2New(0.5, 0, 0.5, 0),
                    Size = UDim2New(1, -84, 1, -110),
                    BackgroundColor3 = FromRGB(26, 25, 31),
                    BorderSizePixel = 0,
                    Visible = false,
                    ZIndex = 81
                })  Items["SuccessPanel"]:AddToTheme({BackgroundColor3 = "Inline"})

                Instances:Create("UICorner", {
                    Parent = Items["SuccessPanel"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 8)
                })

                Items["SuccessIcon"] = Instances:Create("ImageLabel", {
                    Parent = Items["SuccessPanel"].Instance,
                    Name = "\0",
                    Image = "rbxassetid://136434898133814",
                    BackgroundTransparency = 1,
                    Position = UDim2New(0.5, -28, 0, 18),
                    Size = UDim2New(0, 56, 0, 56),
                    BorderSizePixel = 0,
                    ZIndex = 82
                })

                Items["SuccessTitle"] = Instances:Create("TextLabel", {
                    Parent = Items["SuccessPanel"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    TextColor3 = FromRGB(255, 255, 255),
                    Text = "Your suggestion has been submitted and will be reviewed by our staff team.",
                    Position = UDim2New(0, 18, 0, 84),
                    Size = UDim2New(1, -36, 0, 42),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    TextWrapped = true,
                    TextSize = 16,
                    ZIndex = 82
                })  Items["SuccessTitle"]:AddToTheme({TextColor3 = "Text"})

                Items["SuccessDetails"] = Instances:Create("TextLabel", {
                    Parent = Items["SuccessPanel"].Instance,
                    Name = "\0",
                    FontFace = Library.Subfont,
                    TextColor3 = FromRGB(210, 210, 220),
                    Text = "",
                    Position = UDim2New(0, 18, 0, 136),
                    Size = UDim2New(1, -36, 1, -196),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    TextWrapped = true,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    ...
                    TextSize = 13,
                    ZIndex = 82
                })  Items["SuccessDetails"]:AddToTheme({TextColor3 = "Text"})

                Items["SuccessDone"] = Instances:Create("TextButton", {
                    Parent = Items["SuccessPanel"].Instance,
                    Name = "\0",
                    AnchorPoint = Vector2New(1, 1),
                    Position = UDim2New(1, -18, 1, -18),
                    Size = UDim2New(0, 122, 0, 40),
                    FontFace = Library.Font,
                    Text = "Back to Chat",
                    TextColor3 = FromRGB(255, 255, 255),
                    BackgroundColor3 = FromRGB(72, 176, 108),
                ...
                Instances:Create("UICorner", {
                    Parent = Items["SuccessDone"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 6)
                })

                Items["ConfirmCancelOverlay"] = Instances:Create("Frame", {
                    Parent = Items["MailboxPanel"].Instance,
                    Name = "\0",
                    Size = UDim2New(1, 0, 1, 0),
                    BackgroundColor3 = FromRGB(8, 8, 10),
                ...
                Items["ConfirmCancelPanel"] = Instances:Create("Frame", {
                    Parent = Items["ConfirmCancelOverlay"].Instance,
                    Name = "\0",
                    AnchorPoint = Vector2New(0.5, 0.5),
                    Position = UDim2New(0.5, 0, 0.5, 0),
                    Size = UDim2New(1, -120, 0, 180),
                    BackgroundColor3 = FromRGB(26, 25, 31),
                    BorderSizePixel = 0,
                    Visible = false,
                    ZIndex = 71
                })  Items["ConfirmCancelPanel"]:AddToTheme({BackgroundColor3 = "Inline"})

                Instances:Create("UICorner", {
                    Parent = Items["ConfirmCancelPanel"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 8)
                })

                Items["ConfirmCancelTitle"] = Instances:Create("TextLabel", {
                    Parent = Items["ConfirmCancelPanel"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    TextColor3 = FromRGB(255, 255, 255),
                    Text = "Cancel suggestion?",
                    Position = UDim2New(0, 16, 0, 18),
                    Size = UDim2New(1, -32, 0, 22),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    TextSize = 16,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    ZIndex = 72
                })  Items["ConfirmCancelTitle"]:AddToTheme({TextColor3 = "Text"})

                Items["ConfirmCancelBody"] = Instances:Create("TextLabel", {
                    Parent = Items["ConfirmCancelPanel"].Instance,
                    Name = "\0",
                    FontFace = Library.Subfont,
                    TextColor3 = FromRGB(205, 205, 215),
                    Text = "Are you sure you want to cancel this suggestion?",
                    Position = UDim2New(0, 16, 0, 52),
                    Size = UDim2New(1, -32, 0, 54),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    TextWrapped = true,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    ...
                    TextSize = 13,
                    ZIndex = 72
                })  Items["ConfirmCancelBody"]:AddToTheme({TextColor3 = "Text"})

                Items["ConfirmCancelNo"] = Instances:Create("TextButton", {
                    Parent = Items["ConfirmCancelPanel"].Instance,
                    Name = "\0",
                    Position = UDim2New(0, 16, 1, -56),
                    Size = UDim2New(0, 96, 0, 36),
                    FontFace = Library.Font,
                    Text = "No",
                    TextColor3 = FromRGB(255, 255, 255),
                    BackgroundColor3 = FromRGB(70, 70, 82),
                    BorderSizePixel = 0,
                    AutoButtonColor = false,
                    TextSize = 14,
                    ZIndex = 72
                ...
                Instances:Create("UICorner", {
                    Parent = Items["ConfirmCancelNo"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 6)
                })

                Items["ConfirmCancelYes"] = Instances:Create("TextButton", {
                    Parent = Items["ConfirmCancelPanel"].Instance,
                    Name = "\0",
                    AnchorPoint = Vector2New(1, 0),
                    Position = UDim2New(1, -16, 1, -56),
                    Size = UDim2New(0, 126, 0, 36),
                    FontFace = Library.Font,
                    Text = "Yes, Cancel",
                    TextColor3 = FromRGB(255, 255, 255),
                    BackgroundColor3 = FromRGB(182, 72, 72),
                    BorderSizePixel = 0,
                    AutoButtonColor = false,
                    TextSize = 14,
                    ZIndex = 72
                ...
                Instances:Create("UICorner", {
                    Parent = Items["ConfirmCancelYes"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 6)
                })
            end

            local MainPage = SuggestionSystem.Page and SuggestionSystem.Page.Page
            local SuggestionPageItems = SuggestionSystem.Page and SuggestionSystem.Page.Items or nil

            if SuggestionPageItems and not SuggestionPageItems["UnreadBadge"] then
                SuggestionPageItems["UnreadBadge"] = Instances:Create("Frame", {
                    Parent = SuggestionPageItems["Text"].Instance,
                    Name = "\0",
                    AnchorPoint = Vector2New(0, 0.5),
                ...
                Instances:Create("UICorner", {
                    Parent = SuggestionPageItems["UnreadBadge"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(1, 0)
                })

                SuggestionPageItems["UnreadBadgeText"] = Instances:Create("TextLabel", {
                    Parent = SuggestionPageItems["UnreadBadge"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    TextColor3 = FromRGB(255, 255, 255),
            ...
            local SuggestionUnreadBadge = SuggestionPageItems and SuggestionPageItems["UnreadBadge"]
            local SuggestionUnreadBadgeText = SuggestionPageItems and SuggestionPageItems["UnreadBadgeText"]

            local function UpdateReviewUnreadBadges()
                local Count = tonumber(ReviewUnreadCount) or 0
                local Display = Count > 99 and "99+" or tostring(Count)
                local TabWidth = Count > 99 and 22 or (Count >= 10 and 18 or 16)
                local MailWidth = Count > 99 and 26 or (Count >= 10 and 22 or 18)

                if SuggestionUnreadBadge and SuggestionUnreadBadgeText then
                    SuggestionUnreadBadge.Instance.Visible = Count > 0
                    if Count > 0 then
                        SuggestionUnreadBadgeText.Instance.Text = Display
                        SuggestionUnreadBadge.Instance.Size = UDim2New(0, TabWidth, 0, 14)
                    end
                end

                if Items["MailBadge"] and Items["MailBadgeText"] then
                    Items["MailBadge"].Instance.Visible = Count > 0
                    if Count > 0 then
                        Items["MailBadgeText"].Instance.Text = Display
                        Items["MailBadge"].Instance.Size = UDim2New(0, MailWidth, 0, 16)
                    end
                end

                Items["MailButtonIcon"].Instance.Image = Count > 0 and ReviewMailIcon or DefaultMailIcon
            end

            local function ClearReviewUnreadBadges()
                ReviewUnreadCount = 0
                UpdateReviewUnreadBadges()
            end

            local function SaveSeenSuggestionStatuses()
                local Encoded = JsonEncode(SeenSuggestionStatuses)
                if Encoded then
                    writefile(SuggestionSeenCacheFile, Encoded)
                end
            end

            local function MarkCurrentSuggestionsSeen()
                SeenSuggestionStatuses = TableClone(KnownSuggestionStatuses)
                SaveSeenSuggestionStatuses()
                ClearReviewUnreadBadges()
            end

            local function SetSelectedType(Value)
                Draft.Type = Value

                for _, Entry in SuggestionItems["TypeButtons"] do
                    local IsSelected = Entry.Key == Value
                    Entry.Dot.Instance.Visible = IsSelected
                    Entry.Button:Tween(nil, {
                        BackgroundColor3 = IsSelected and Library.Theme["Hovered Element"] or Library.Theme["Element"]
                    })
                end
            end

            local function UpdateCounter()
                local MaxLength = tonumber(SuggestionConfig.SuggestionMaxLength) or 300
                local Text = tostring(Items["SuggestionInput"].Instance.Text or "")
                if StringLen(Text) > MaxLength then
                    Text = StringSub(Text, 1, MaxLength)
                    Items["SuggestionInput"].Instance.Text = Text
                end

                Draft.Content = Text
                local Used = StringLen(Text)
                Items["CounterLabel"].Instance.Text = StringFormat("%d/%d", Used, MaxLength)
                Items["CounterLabel"].Instance.TextColor3 = Used >= MaxLength and FromRGB(255, 112, 112) or FromRGB(170, 170, 185)
            end

            local function UpdateSuggestionInputScroll(ScrollToBottom)
                local ViewHeight = Items["SuggestionInputScroll"].Instance.AbsoluteWindowSize.Y
                if ViewHeight <= 0 then
                    ViewHeight = Items["SuggestionInputScroll"].Instance.AbsoluteSize.Y
                end

                local TextHeight = math.max(138, Items["SuggestionInput"].Instance.TextBounds.Y + 18)
                Items["SuggestionInput"].Instance.Size = UDim2New(1, -6, 0, TextHeight)
                Items["SuggestionInputScroll"].Instance.CanvasSize = UDim2New(0, 0, 0, TextHeight)

                if ScrollToBottom and TextHeight > ViewHeight then
                    Items["SuggestionInputScroll"].Instance.CanvasPosition = Vector2New(0, TextHeight - ViewHeight)
                elseif TextHeight <= ViewHeight then
                    Items["SuggestionInputScroll"].Instance.CanvasPosition = Vector2New(0, 0)
                end
            end

            local function ResetDraft(KeepGameTitle)
                Draft.Content = ""
                Draft.Type = ""
                if KeepGameTitle then
                    Draft.GameTitle = Draft.GameTitle
                else
                    Draft.GameTitle = GetResolvedGameName(function(ResolvedName)
                        if Draft.GameTitle == "Ugc" or Draft.GameTitle == "" then
                            Draft.GameTitle = ResolvedName
                            Items["GameTitleInput"].Instance.Text = ResolvedName
                        end
                    end)
                    if Draft.GameTitle == "" then
                        Draft.GameTitle = "This Experience"
                    end
                end
                Items["GameTitleInput"].Instance.Text = Draft.GameTitle
                Items["SuggestionInput"].Instance.Text = ""
                SetSelectedType(Draft.Type)
                UpdateCounter()
                UpdateSuggestionInputScroll(false)
            end

            local function SwitchToChatPage()
                if Data.ChatPage and type(Data.ChatPage.Turn) == "function" then
                    for _, Value in Data.ChatPage.Page.SubPages do
                        Value:Turn(Value == Data.ChatPage)
                    end
                end
            end

            local OpenCancelConfirm

            local function RenderSuggestionEntries(Entries)
                ClearSuggestionRows()
                Items["MailboxEmpty"].Instance.Visible = #Entries == 0
                ...
                for _, Entry in Entries do
                    ...
                    local EntryStatus = tostring(Entry.status or "under_review")
                    SeenStatuses[EntryId] = EntryStatus

                    if EntryStatus ~= "under_review" and SeenSuggestionStatuses[EntryId] ~= EntryStatus then
                        UnreadCount = UnreadCount + 1
                    end
                end

                KnownSuggestionStatuses = SeenStatuses
                ReviewUnreadCount = UnreadCount

                if SuggestionSystem.Page and SuggestionSystem.Page.Active then
                    SeenSuggestionStatuses = TableClone(KnownSuggestionStatuses)
                    SaveSeenSuggestionStatuses()
                    ReviewUnreadCount = 0
                end
                ...
                for _, Entry in Entries do
                    local Row = Instances:Create("Frame", {
                        Parent = Items["MailboxScroller"].Instance,
                        Name = "\0",
                        Size = UDim2New(1, -2, 0, 92),
                        BorderSizePixel = 0,
                        BackgroundColor3 = FromRGB(41, 40, 49),
                        ZIndex = 63
                    })  Row:AddToTheme({BackgroundColor3 = "Element"})
                    ...
                    local Status = tostring(Entry.status or "under_review")
                    local IconId = GetStatusIcon(Status)
                    local CanCancel = Status == "under_review"
                    local TitleWidthOffset = CanCancel and 94 or 24

                    local IconSize = 16
                    local StatusIcon = Instances:Create("ImageLabel", {
                        Parent = Row.Instance,
                        Name = "\0",
                        Image = IconId,
                        BackgroundTransparency = 1,
                        AnchorPoint = Vector2New(0, 0.5),
                        Position = UDim2New(0, 12, 1, -18),
                    ...
                    local Title = Instances:Create("TextLabel", {
                        ...
                        FontFace = Library.Font,
                        TextColor3 = FromRGB(255, 255, 255),
                        Text = tostring(Entry.gameTitle or "Unknown Game"),
                        Position = UDim2New(0, 12, 0, 10),
                        Size = UDim2New(1, -TitleWidthOffset, 0, 18),
                        BackgroundTransparency = 1,
                        BorderSizePixel = 0,
                        TextSize = 14,
                        TextXAlignment = Enum.TextXAlignment.Left,
                    ...
                    local Preview = Instances:Create("TextLabel", {
                        Parent = Row.Instance,
                        Name = "\0",
                        FontFace = Library.Subfont,
                        TextColor3 = FromRGB(185, 185, 195),
                        Text = TruncatePreview(Entry.content, 120),
                        Position = UDim2New(0, 12, 0, 32),
                        Size = UDim2New(1, -24, 0, 34),
                        BackgroundTransparency = 1,
                        BorderSizePixel = 0,
                        TextWrapped = true,
                        TextXAlignment = Enum.TextXAlignment.Left,
                        ...
                        TextSize = 12,
                        ZIndex = 64
                    })  Preview:AddToTheme({TextColor3 = "Text"})

                    local RejectReason = Status == "rejected" and tostring(Entry.rejectReason or "") or ""
                    local StatusLabel = GetStatusLabel(Status)
                    if RejectReason ~= "" then
                        StatusLabel = StatusLabel .. " - " .. RejectReason
                    end

                    local StatusText = Instances:Create("TextLabel", {
                        Parent = Row.Instance,
                        Name = "\0",
                        FontFace = Library.Subfont,
                        TextColor3 = GetStatusColor(Status),
                        Text = StatusLabel,
                        Position = UDim2New(0, IconId ~= "" and 34 or 12, 1, -26),
                        Size = UDim2New(1, -(IconId ~= "" and 46 or 24), 0, 16),
                        BackgroundTransparency = 1,
                        BorderSizePixel = 0,
                        TextXAlignment = Enum.TextXAlignment.Left,
                        TextTruncate = Enum.TextTruncate.AtEnd,
                    ...
                    local CancelPreviewButton = Instances:Create("TextButton", {
                        Parent = Row.Instance,
                        Name = "\0",
                        FontFace = Library.Font,
                        TextColor3 = FromRGB(255, 255, 255),
                    ...
                    Instances:Create("UICorner", {
                        Parent = CancelPreviewButton.Instance,
                        Name = "\0",
                        CornerRadius = UDimNew(0, 5)
                    })

                    CancelPreviewButton:Connect("InputBegan", function(Input)
                        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                            OpenCancelConfirm(true, Entry)
                        end
                    end)

                    CancelPreviewButton:OnHover(function()
                        CancelPreviewButton:Tween(TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                            BackgroundColor3 = FromRGB(202, 86, 86),
                            Size = UDim2New(0, 62, 0, 20)
                        })
                    end)

                    CancelPreviewButton:OnHoverLeave(function()
                        CancelPreviewButton:Tween(TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                            BackgroundColor3 = FromRGB(182, 72, 72),
                            Size = UDim2New(0, 58, 0, 20)
                        })
                    ...
                    local TypeText = Instances:Create("TextLabel", {
                        Parent = Row.Instance,
                        Name = "\0",
                        FontFace = Library.Subfont,
                        TextColor3 = FromRGB(170, 170, 185),
                        Text = GetSuggestionTypeLabel(tostring(Entry.type or "suggestion")),
                        AnchorPoint = Vector2New(1, 1),
                        Position = UDim2New(1, -12, 1, -12),
                        Size = UDim2New(0, 0, 0, 16),
                        AutomaticSize = Enum.AutomaticSize.X,
                    ...
                    TableInsert(SuggestionRows, Row)
                end
            end

            function SuggestionSystem:RefreshSuggestions()
                local Config = SuggestionConfig
                if not Config or not Config.HttpUrl or Config.HttpUrl == "" or not RequestFunction then
                    return false
                end

                local Ok, Body = DoHttpRequest({
                    Url = BuildSuggestionAuthorizedUrl(Config, Config.HttpUrl .. "/library/suggestions", {
                        senderId = tostring(LocalPlayer.UserId),
                        limit = tonumber(Config.SuggestionMailboxLimit) or 20,
                    }),
                    Method = "GET",
                    Headers = GetSuggestionHeaders(Config),
                })
                ...
                local Decoded = JsonDecode(Body)
                if type(Decoded) ~= "table" or type(Decoded.suggestions) ~= "table" then
                    return false
                end

                RenderSuggestionEntries(Decoded.suggestions)
                return true
            end

            local function OpenMailbox(Bool)
                MailboxOpen = Bool == true
                SetOverlayVisible(Items["MailboxOverlay"], Items["MailboxPanel"], Bool)
                if Bool then
                    SuggestionSystem:RefreshSuggestions()
                end
            end

            local function OpenSuccessOverlay(Bool)
                SetOverlayVisible(Items["SuccessOverlay"], Items["SuccessPanel"], Bool)
            end

            OpenCancelConfirm = function(Bool, Suggestion)
                PendingCancelSuggestion = Bool and Suggestion or nil
                SetOverlayVisible(Items["ConfirmCancelOverlay"], Items["ConfirmCancelPanel"], Bool)
            end

            local function FormatCooldownNotice(RetryAfterMs)
                local TotalSeconds = math.max(0, math.floor((tonumber(RetryAfterMs) or 0) / 1000))
                local Minutes = math.floor(TotalSeconds / 60)
                local Seconds = TotalSeconds % 60
                return StringFormat("You must wait %d minute%s, %d second%s before submitting another suggestion.", Minutes, Minutes == 1 and "" or "s", Seconds, Seconds == 1 and "" or "s")
            end

            local function CancelSuggestion(Entry)
                local SuggestionId = tonumber(Entry and Entry.id)
                ...
                if not Config or not Config.HttpUrl or Config.HttpUrl == "" or not RequestFunction then
                    Library:MakeNotification({
                        ...
                        Content = "Suggestion backend is not configured.",
                        Time = 3
                    })
                    return
                end

                Items["ConfirmCancelYes"].Instance.Text = "Cancelling..."
                local Ok, Body = DoHttpRequest({
                    Url = BuildSuggestionAuthorizedUrl(Config, Config.HttpUrl .. "/library/suggestions/" .. tostring(SuggestionId), {
                        senderId = tostring(LocalPlayer.UserId),
                    }),
                    Method = "DELETE",
                    Headers = GetSuggestionHeaders(Config),
                })
                Items["ConfirmCancelYes"].Instance.Text = "Yes, Cancel"

                local Decoded = type(Body) == "string" and JsonDecode(Body) or nil
                if not Ok then
                    Library:MakeNotification({
                        Name = "Suggestion Error",
                        Content = tostring((Decoded and Decoded.error) or Body or "Failed to cancel suggestion."),
                        Time = 4
                    })
                    return
                end
                ...
                SuggestionSystem:RefreshSuggestions()
                Library:MakeNotification({
                    Name = "Suggestion Cancelled",
                    Content = "Your suggestion was canceled successfully.",
                    Time = 3
                })
            end

            local function BuildSuccessDetails(Suggestion)
                local SubmittedAt = OsDate("%Y-%m-%d %I:%M:%S %p", math.floor((tonumber(Suggestion.createdAt) or os.time() * 1000) / 1000))
                return TableConcat({
                    "User ID: " .. tostring(Suggestion.senderId or LocalPlayer.UserId),
                    "Display Name: " .. tostring(Suggestion.displayName or LocalPlayer.DisplayName),
                    "@Username: @" .. tostring(Suggestion.username or LocalPlayer.Name),
                    "Game Title: " .. tostring(Suggestion.gameTitle or "Unknown Game"),
                    "Suggestion Type: " .. GetSuggestionTypeLabel(tostring(Suggestion.type or "suggestion")),
                    "Suggestion Details: " .. tostring(Suggestion.content or ""),
                    "Executor: " .. tostring(Suggestion.executor or "Unknown"),
                    "Submitted At: " .. SubmittedAt,
                }, "\n\n")
            end

            local function SubmitSuggestion()
                local GameTitle = (Items["GameTitleInput"].Instance.Text or ""):match("^%s*(.-)%s*$") or ""
                local ContentText = (Items["SuggestionInput"].Instance.Text or ""):match("^%s*(.-)%s*$") or ""
                local Minimum = tonumber(SuggestionConfig.SuggestionMinLength) or 5
                ...
                if StringLen(ContentText) < Minimum then
                    Library:MakeNotification({
                        Name = "Suggestion Error",
                        Content = StringFormat("Suggestion details must be at least %d characters.", Minimum),
                        Time = 3
                    })
                    return
                end
                ...
                if Draft.Type == "" then
                    Library:MakeNotification({
                        Name = "Suggestion Error",
                        Content = "Choose one suggestion type before submitting.",
                        Time = 3
                    })
                    return
                end
                ...
                Items["SubmitButton"].Instance.Text = "Submitting..."
                local Encoded = JsonEncode(Payload)
                if not Encoded then
                    Items["SubmitButton"].Instance.Text = "Submit"
                    return
                end
                ...
                local Ok, Body = DoHttpRequest({
                    Url = BuildSuggestionAuthorizedUrl(Config, Config.HttpUrl .. "/library/suggestions"),
                    Method = "POST",
                    Headers = MergeTable(GetSuggestionHeaders(Config), {
                        ["Content-Type"] = "application/json",
                    }),
                    Body = Encoded,
                })
                ...
                if not Ok then
                    local ErrorText = tostring((Decoded and Decoded.error) or Body or "Failed to submit suggestion.")
                    if type(Decoded) == "table" and tonumber(Decoded.retryAfterMs) then
                        ErrorText = FormatCooldownNotice(Decoded.retryAfterMs)
                    end
                ...
                if type(Decoded) ~= "table" or Decoded.ok ~= true or type(Decoded.suggestion) ~= "table" then
                    Library:MakeNotification({
                        Name = "Suggestion Error",
                        Content = tostring((Decoded and Decoded.error) or Body or "Failed to submit suggestion."),
                        Time = 4
                    })
                    return
                end

                SuccessPayload = Decoded.suggestion
                Items["SuccessDetails"].Instance.Text = BuildSuccessDetails(SuccessPayload)
                SuggestionSystem:RefreshSuggestions()
                ResetDraft(false)
                OpenSuccessOverlay(true)
            end

            Items["MailButton"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    OpenMailbox(true)
                end
            end)

            Items["MailboxClose"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    OpenMailbox(false)
                end
            end)

            Items["ConfirmCancelNo"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    OpenCancelConfirm(false)
                end
            end)

            Items["ConfirmCancelYes"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    CancelSuggestion(PendingCancelSuggestion)
                end
            end)

            Items["SuccessDone"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            ...
            for _, Entry in SuggestionItems["TypeButtons"] do
                Entry.Button:Connect("InputBegan", function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        SetSelectedType(Entry.Key)
                    end
                end)
            end

            Items["GameTitleInput"].Instance:GetPropertyChangedSignal("Text"):Connect(function()
                Draft.GameTitle = Items["GameTitleInput"].Instance.Text
            end)

            Items["SuggestionInput"].Instance:GetPropertyChangedSignal("Text"):Connect(function()
                UpdateCounter()
                UpdateSuggestionInputScroll(true)
            end)

            Items["SuggestionInputScroll"].Instance:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
                UpdateSuggestionInputScroll(false)
            end)

            Items["CancelButton"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    ResetDraft(false)
                    SwitchToChatPage()
                end
            ...
            Items["SubmitButton"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    SubmitSuggestion()
                end
            end)

            if SuggestionSystem.Page and SuggestionSystem.Page.Items and SuggestionSystem.Page.Items["Inactive"] then
                Library:Connect(SuggestionSystem.Page.Items["Inactive"].Instance.InputBegan, function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        task.defer(function()
                            if SuggestionSystem:RefreshSuggestions() then
                                MarkCurrentSuggestionsSeen()
                            end
                        end)
                    end
                end)
            ...
            Library:Thread(function()
                while not SuggestionDestroyed do
                    task.wait(20)
                    pcall(function()
                        SuggestionSystem:RefreshSuggestions()
                    end)
        ...
        Library.Pages.AddSection = function(self, Data)
            local Section = {
                ...
                Premium = Data.Premium or Data.premium or Data.Blocked == true or Data.blocked == true or type(Data.Blocked) == "string" or type(Data.blocked) == "string",
                Blocked = Data.Blocked == true or Data.blocked == true or type(Data.Blocked) == "string" or type(Data.blocked) == "string",
                BlockedText = (type(Data.Blocked) == "string" and Data.Blocked) or (type(Data.blocked) == "string" and Data.blocked) or Data.BlockedText or Data.blockedText or "Premium Access Only!",
                IsOpen = false,

                Items = { }
            }

            if Section.Page.Blocked or Section.Blocked then
                Section.IsOpen = true
            end
            ...
            local Items = { } do 
                ...
                local SectionIconData

                if type(Section.Icon) == "string" and Section.Icon ~= "" then
                    if Section.Icon:find("rbxassetid://", 1, true) or Section.Icon:find("rbxasset://", 1, true) or Section.Icon:find("http", 1, true) then
                        SectionIconData = {Url = Section.Icon}
                    elseif Lucide then
                        local Success, IconData = pcall(function()
                            return Lucide.GetAsset(Section.Icon, 48)
                        end)
                        SectionIconData = Success and IconData or nil
                    end
                end
                ...
                if SectionIconData then
                    Items["Icon"] = Instances:Create("ImageLabel", {
                        Parent = Items["Section"].Instance,
                        Name = "\0",
                        Image = SectionIconData.Url,
                        ImageRectSize = SectionIconData.ImageRectSize,
                        ImageRectOffset = SectionIconData.ImageRectOffset,
                ...
                Items["Text"] = Instances:Create("TextLabel", {
                    ...
                    BackgroundTransparency = 1,
                    AnchorPoint = Vector2New(0, 0.5),
                    Position = UDim2New(0, SectionIconData and 32 or 8, 0, 17),
                    BorderSizePixel = 0,
                    AutomaticSize = Enum.AutomaticSize.X,
                    TextSize = 15,
                    BackgroundColor3 = FromRGB(255, 255, 255)
                ...
                if Section.Premium then
                    Items["IsPremium"].Instance.Size = UDim2New(1, -4, 1, -4)
                    Items["IsPremium"].Instance.Position = UDim2New(0, 2, 0, 2)
                ...
                function Section:SetOpen(Bool)
                    if Section.Blocked or Section.Page.Blocked then
                        Bool = true
                    end
                    self.IsOpen = Bool
        ...
        Library.Sections.AddToggle = function(self, Data)
            ...
            local Toggle = {
                ...
                Warning = Data.Warning or Data.warning or false
            }
            ...
            local EmptyKeybind = Data.Keybind == "Nothing" or Data.Keybind == "None" or Data.DefaultKeybind == "Nothing" or Data.DefaultKeybind == "None" or Data.defaultKeybind == "Nothing" or Data.defaultKeybind == "None"
            local HasInlineKeybind = Data.Keybind ~= nil or Data.DefaultKeybind ~= nil or Data.defaultKeybind ~= nil
            local IsHovered = false
            local UpdateToggleVisuals
            local InlineKeybind, InlineKeybindItems
            ...
            local Items = { } do
                ...
                if Toggle.Warning then
                    Items["Text"].Instance.Text = Toggle.Name .. " ⚠️"
                    Items["Text"].Instance.TextColor3 = FromRGB(255, 200, 0)

                    local WarnHitbox = InstanceNew("TextButton")
                    WarnHitbox.Name = "\0"
                    WarnHitbox.Text = ""
                    WarnHitbox.BackgroundTransparency = 1
                    WarnHitbox.BorderSizePixel = 0
                    WarnHitbox.Size = UDim2New(0, 22, 1, 0)
                    WarnHitbox.AnchorPoint = Vector2New(1, 0)
                    WarnHitbox.Position = UDim2New(0, 0, 0, 0)
                    WarnHitbox.ZIndex = 5
                    WarnHitbox.AutoButtonColor = false
                    WarnHitbox.Parent = Items["Text"].Instance

                    local function UpdateHitboxPos()
                        WarnHitbox.Position = UDim2New(1, 0, 0, 0)
                    end
                    Items["Text"].Instance:GetPropertyChangedSignal("AbsoluteSize"):Connect(UpdateHitboxPos)
                    UpdateHitboxPos()

                    local WarnTooltip = Instances:Create("Frame", {
                        Parent = Library.Holder.Instance, Name = "\0",
                        BackgroundColor3 = FromRGB(15, 12, 16), BorderSizePixel = 0,
                        Size = UDim2New(0, 0, 0, 0), BackgroundTransparency = 1,
                        AutomaticSize = Enum.AutomaticSize.XY, ZIndex = 103,
                    })  WarnTooltip:AddToTheme({BackgroundColor3 = "Background"})

                    Instances:Create("UISizeConstraint", {
                        Parent = WarnTooltip.Instance, Name = "\0",
                        MinSize = Vector2New(0, 0), MaxSize = Vector2New(235, math.huge)
                    })

                    local WarnText = Instances:Create("TextLabel", {
                        Parent = WarnTooltip.Instance, Name = "\0",
                        FontFace = Library.Font, TextColor3 = FromRGB(255, 255, 255),
                        BorderColor3 = FromRGB(0, 0, 0), Text = "This feature is risky, use with caution!",
                        BorderSizePixel = 0, BackgroundTransparency = 1,
                        Size = UDim2New(1, 0, 1, 0), ZIndex = 103, TextTransparency = 1,
                        AutomaticSize = Enum.AutomaticSize.Y, TextXAlignment = Enum.TextXAlignment.Left,
                        TextWrapped = true, TextSize = 14, BackgroundColor3 = FromRGB(15, 12, 16)
                    })  WarnText:AddToTheme({TextColor3 = "Text"})

                    Instances:Create("UIPadding", {
                        Parent = WarnText.Instance, Name = "\0",
                        PaddingBottom = UDimNew(0, 6), PaddingLeft = UDimNew(0, 6),
                        PaddingRight = UDimNew(0, 6), PaddingTop = UDimNew(0, 6),
                    })

                    local WarnStroke = Instances:Create("UIStroke", {
                        Parent = WarnTooltip.Instance, Name = "\0",
                        Color = FromRGB(255, 200, 0), Thickness = 1.2, Transparency = 1,
                        LineJoinMode = Enum.LineJoinMode.Round, ApplyStrokeMode = Enum.ApplyStrokeMode.Border
                    })

                    Instances:Create("UICorner", {
                        Parent = WarnTooltip.Instance, Name = "\0", CornerRadius = UDimNew(0, 5)
                    })

                    local WarnAnimThread, InputChangedConn

                    local function UpdateWarnPos()
                        local ML = UserInputService:GetMouseLocation()
                        WarnTooltip.Instance.Position = UDim2New(0, ML.X + 8, 0, ML.Y - 35)
                    end

                    local function HideWarnTooltip()
                        WarnTooltip:Tween(nil, {BackgroundTransparency = 1})
                        WarnText:Tween(nil, {TextTransparency = 1})
                        WarnStroke:Tween(nil, {Transparency = 1})
                        if WarnAnimThread then task.cancel(WarnAnimThread) WarnAnimThread = nil end
                        if InputChangedConn then InputChangedConn:Disconnect() InputChangedConn = nil end
                    end

                    WarnHitbox.MouseEnter:Connect(function()
                        if Library.HideTooltips or Library.ActiveBlockedSubTab then return end
                        UpdateWarnPos()
                        WarnTooltip:Tween(nil, {BackgroundTransparency = 0})
                        WarnText:Tween(nil, {TextTransparency = 0})
                        WarnStroke:Tween(nil, {Transparency = 0})
                        ...
                        if WarnAnimThread then task.cancel(WarnAnimThread) end
                        WarnAnimThread = task.spawn(function()
                            local toWhite = true
                            while true do
                                WarnStroke:Tween(TweenInfo.new(0.8, Enum.EasingStyle.Sine), {Color = toWhite and FromRGB(255, 255, 255) or FromRGB(255, 200, 0)})
                                toWhite = not toWhite
                                task.wait(0.8)
                            end
                        end)
                    ...
                    WarnHitbox.MouseLeave:Connect(function()
                        HideWarnTooltip()
                    end)
                end
                ...
                UpdateToggleVisuals = function()
                    ...
                    if IsCheckbox then
                        ...
                        Items["Check"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                            ...
                            Size = Toggle.Value and UDim2New(0, 8, 0, 8) or UDim2New(0, 0, 0, 0)
                        })
                    else
                        Items["Check"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                            ImageTransparency = 1,
                        ...
                        Items["Knob"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
                            Position = Toggle.Value and UDim2New(0, 17, 0.5, 0) or UDim2New(0, 3, 0.5, 0),
                            Size = UDim2New(0, 12, 0, 12)
                        })
                    end
                end
            ...
            function Toggle:Set(Bool)
                ...
                if InlineKeybind then
                    InlineKeybind.Toggled = Bool
                    Library.Flags[InlineKeybind.Flag] = {
                        Mode = InlineKeybind.Mode,
                        Key = InlineKeybind.Key,
                        Toggled = InlineKeybind.Toggled
                    }
                end
                Library:QueueAutoSave()
            ...
            if HasInlineKeybind then
                InlineKeybind, InlineKeybindItems = Library:CreateKeybind({
                    Parent = Items["SubElements"],
                    Flag = Data.KeybindFlag or Data.keybindFlag or (Toggle.Flag .. "Keybind"),
                    Premium = Toggle.Premium,
                    Default = EmptyKeybind and Enum.KeyCode.Backspace or (Data.Keybind or Data.DefaultKeybind or Data.defaultKeybind),
                    Mode = Data.Mode or Data.mode or "Toggle",
                    Callback = function(Value)
                        Toggle:Set(Value)
                    end
                })
                InlineKeybindItems["KeyButton"].Instance.LayoutOrder = 999
            end
            ...
            Items["Toggle"]:Connect("MouseButton1Click", function()
                ...
                if (not InlineKeybindItems or not Library:IsMouseOverFrame(InlineKeybindItems["KeyButton"])) and (Library:IsMouseOverFrame(Items["Text"]) or Library:IsMouseOverFrame(Items["Indicator"])) then
                    Toggle:Set(not Toggle.Value)
                end
            end)
        ...
        Library.Sections.AddDropdown = function(self, Data)
            ...
            function Dropdown:UpdateMultiSelectAllState()
                if not self.Multi or not Items["SelectAllIndicator"] then
                    return
                end
            ...
            function Dropdown:ApplyMultiSelection(Options, ShouldSave, ShouldCallback)
                ...
                if ShouldCallback ~= false and self.Callback then
                    Library:SafeCall(self.Callback, self.Value)
                end
            end
            ...
            function Dropdown:SetOpen(Bool)
                ...
                if self.IsOpen then 
                    ...
                    if type(Dropdown.Options) == "function" or (type(Dropdown.Options) == "Instance" and (Dropdown.Options.ClassName == "Players" or Dropdown.Options:IsA("DataModel"))) then
                        ...
                        OptionsRefreshThread = task.spawn(function()
                            while Dropdown.IsOpen and OptionsRefreshToken == CurrentToken do
                                ...
                                if not Dropdown.IsOpen or OptionsRefreshToken ~= CurrentToken then
                                    break
                                end
                    ...
                    if OptionsRefreshThread then
                        pcall(function()
                            task.cancel(OptionsRefreshThread)
                        end)
                        OptionsRefreshThread = nil
                    end
            ...
            function Dropdown:Remove(Option)
                if Dropdown.OptionInstances[Option] then 
                    ...
                    if OrderIndex then
                        TableRemove(Dropdown.OptionOrder, OrderIndex)
                    end

                    if Dropdown.Multi then
                        Dropdown:ApplyMultiSelection(Dropdown.Value, false, false)
                    end
                end
            end
    ...
    Library.CreateConfigsSection = function(self, Tab)
            ...
            Callback = function()
                if ConfigSelected then
                    ...
                    if not Success then
                        ...
                        Library:MakeNotification({
                            Name = "Success!",
                            Content = Message and ("Loaded config "..ConfigSelected.." with warnings\n"..Message) or ("Loaded config "..ConfigSelected.." successfully"),
                            Time = 3
                        })
                    end
                end
        ...
        ConfigsSection:AddToggle({
            ...
            Callback = function(Value)
                ...
                if not AutoSaveToggleReady then
                    return
                end
        ...
        ConfigsSection:AddButton({
            ...
            Callback = function()
                ...
                if type(CurrentConfig) ~= "string" or CurrentConfig == "" then
                    Library:MakeNotification({
                        Name = "Error!",
                        Content = "Failed to build the current config",
                        Time = 3
                    })
                    return
                end
                ...
                if type(ClipboardSetter) == "function" then
                    Copied = pcall(ClipboardSetter, CurrentConfig)
                end

                Library:MakeNotification({
                    Name = "Success!",
                    Content = Copied and "Current config copied to clipboard" or "Current config added to the textbox",
                    Time = 3
                })
            end
        })
        ...
        ConfigsSection:AddButton({
            ...
            Callback = function()
                ...
                if type(PastedConfig) ~= "string" or PastedConfig == "" then
                    Library:MakeNotification({
                        Name = "Error!",
                        Content = "Paste a shared config first",
                        Time = 3
                    })
                    return
                end
                ...
                if not Success then
                    Library:MakeNotification({
                        Name = "Error!",
                        Content = "Failed to import shared config:\n" .. tostring(Message),
                        Time = 5
                    })
                    return
                end
                ...
                Library:MakeNotification({
                    Name = "Success!",
                    Content = Message and ("Imported shared config with warnings\n" .. Message) or "Imported shared config successfully",
                    Time = 10
                })
            end
        })
        ...
        Library._PendingAutoload = function()
            ...
            if Library:IsAutoSaveEnabled() then
                ...
                if Success then
                    ...
                    Library:MakeNotification({
                        Name = "Config Loaded",
                        Content = Message and ("Loaded your last saved config with warnings\n" .. Message) or "Loaded your last saved config",
                        Time = 3
                    })
                elseif Message then
                    Library:MakeNotification({
                        Name = "Config Loaded",
                        Content = "Failed to load your last saved config: " .. tostring(Message),
                        Time = 5
                    })
                end
            end
                ...
                if AutoloadName then
                    ...
                    if isfile(FilePath) then
                        ...
                        if Success then
                            ...
                            Library:MakeNotification({
                                Name = "Autoload",
                                Content = Message and ("Loaded config " .. AutoloadName .. " with warnings\n" .. Message) or ("Loaded config " .. AutoloadName .. " automatically"),
                                Time = 3
                            })
                        else
                            Library:MakeNotification({
                                Name = "Autoload",
                                Content = "Failed to autoload config " .. AutoloadName .. ": " .. tostring(Message),
                                Time = 5
                            })
                        end
                    else
    ...
    Library.CreateSettingsSection = function(self, Tab)
        ...
        local function AddExpressSettingToggle(Name, Flag, Key)
            SettingsSection:AddToggle({
                ...
                Callback = function(Value)
                    Library.ExpressExtraSettings[Key] = Value
                end
            })
        end
        ...
        SettingsSection:AddToggle({
            ...
            Default = false,
            Callback = function(Value)
                Library.WindowItem:SetStreamerMode(Value)
            end
        })
    ...
    Library.CreateHomeTab = function(self, Window, DiscordLink)
        ...
        local function AddHomeCorner(Parent, Radius)
            Instances:Create("UICorner", {
                Parent = Parent,
                Name = "\0",
                CornerRadius = UDimNew(0, Radius or 12)
            })
        end
        ...
        local Items = { } do
            ...
            AddHomeCorner(Items["SessionInfo"].Instance)
            ...
            AddHomeCorner(Items["Players"].Instance)
            ...
            AddHomeCorner(Items["MaxPlayers"].Instance)
            ...
            AddHomeCorner(Items["AccountInformation"].Instance)
            ...
            if Library.Flags.StreamerMode then
                Window:SetStreamerMode(true)
            end
            ...
            AddHomeCorner(Items["Discord"].Instance)
            ...
            Items["JoinDiscord"]:Connect("MouseEnter", function()
                Items["JoinDiscord"]:Tween(nil, {
                    BackgroundColor3 = FromRGB(102, 115, 255),
                    Size = UDim2New(0.29, 0, 0, 38)
                })
            end)
            ...
            AddHomeCorner(Items["ExecutorInfo"].Instance)
            ...
            local unsupported = exec:find("xeno") or exec:find("solara") or exec:find("velocity") or exec:find("real")
            local supportColor = unsupported and FromRGB(255, 170, 64) or FromRGB(67, 214, 117)

            Items["ExecutorStatusPulse1"] = Instances:Create("Frame", {
                Parent = Items["ExecutorInfo"].Instance,
                Name = "\0",
                AnchorPoint = Vector2New(0.5, 0.5),
                Position = UDim2New(0, 18, 0, 41),
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(0, 8, 0, 8),
                BorderSizePixel = 0,
            ...
            AddHomeCorner(Items["ExecutorStatusPulse1"].Instance, 24)

            Items["ExecutorStatusPulse2"] = Instances:Create("Frame", {
                Parent = Items["ExecutorInfo"].Instance,
                Name = "\0",
                AnchorPoint = Vector2New(0.5, 0.5),
                Position = UDim2New(0, 18, 0, 41),
            ...
            AddHomeCorner(Items["ExecutorStatusPulse2"].Instance, 24)

            Items["ExecutorStatusDot"] = Instances:Create("Frame", {
                Parent = Items["ExecutorInfo"].Instance,
                Name = "\0",
                AnchorPoint = Vector2New(0, 0.5),
            ...
            AddHomeCorner(Items["ExecutorStatusDot"].Instance, 8)

            local function AnimateExecutorPulse(Item, Delay)
                Library:Thread(function()
                    task.wait(Delay)

                    local Pulse = Item.Instance

                    while Library and Pulse and Pulse.Parent do
                        Pulse.Size = UDim2New(0, 8, 0, 8)
                        Pulse.BackgroundTransparency = 0.45

                        Tween:Create(Pulse, TweenInfo.new(1.65, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            ...
            AnimateExecutorPulse(Items["ExecutorStatusPulse1"], 0)
            AnimateExecutorPulse(Items["ExecutorStatusPulse2"], 0.8)
            ...
            Items["PerformanceInfo"] = Instances:Create("Frame", {
                Parent = Items["HomeScroll"].Instance,
                Name = "\0",
                ...
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(26, 25, 31)
            })  Items["PerformanceInfo"]:AddToTheme({BackgroundColor3 = "Inline"})

            AddHomeCorner(Items["PerformanceInfo"].Instance)

            Items["PerformanceTitle"] = Instances:Create("TextLabel", {
                Parent = Items["HomeScroll"].Instance,
                Name = "\0",
                FontFace = Library.Font,
                TextColor3 = FromRGB(255, 255, 255),
                ...
                TextSize = 12,
                BackgroundColor3 = FromRGB(255, 255, 255)
            })  Items["PerformanceTitle"]:AddToTheme({TextColor3 = "Text"})
            ...
            Items["FPSCard"] = Instances:Create("Frame", {
                ...
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(17, 16, 20)
            })  Items["FPSCard"]:AddToTheme({BackgroundColor3 = "Background"})

            AddHomeCorner(Items["FPSCard"].Instance)

            Items["FPSAccent"] = Instances:Create("Frame", {
                Parent = Items["FPSCard"].Instance,
                Name = "\0",
                ...
                BorderSizePixel = 0,
                BackgroundColor3 = Library.Theme.Accent
            })  Items["FPSAccent"]:AddToTheme({BackgroundColor3 = "Accent"})

            Instances:Create("UICorner", {
                Parent = Items["FPSAccent"].Instance,
                Name = "\0",
                CornerRadius = UDimNew(1, 0)
            })

            Items["FPSValue"] = Instances:Create("TextLabel", {
                Parent = Items["FPSCard"].Instance,
                Name = "\0",
                FontFace = Library.Font,
                TextColor3 = FromRGB(255, 255, 255),
                ...
                TextSize = 13,
                BackgroundColor3 = FromRGB(255, 255, 255)
            })  Items["FPSValue"]:AddToTheme({TextColor3 = "Text"})

            Items["FPSDescription"] = Instances:Create("TextLabel", {
                Parent = Items["FPSCard"].Instance,
                Name = "\0",
                FontFace = Library.Font,
                TextColor3 = FromRGB(255, 255, 255),
                ...
                TextSize = 12,
                BackgroundColor3 = FromRGB(255, 255, 255)
            })  Items["FPSDescription"]:AddToTheme({TextColor3 = "Text"})

            Items["PingCard"] = Instances:Create("Frame", {
                Parent = Items["PerformanceInfo"].Instance,
                Name = "\0",
                AnchorPoint = Vector2New(1, 0),
                ...
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(17, 16, 20)
            })  Items["PingCard"]:AddToTheme({BackgroundColor3 = "Background"})

            AddHomeCorner(Items["PingCard"].Instance)

            Items["PingAccent"] = Instances:Create("Frame", {
                Parent = Items["PingCard"].Instance,
                Name = "\0",
                Position = UDim2New(0, 8, 0, 12),
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(0, 3, 0, 30),
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(88, 113, 255)
            })

            Instances:Create("UICorner", {
                Parent = Items["PingAccent"].Instance,
                Name = "\0",
                CornerRadius = UDimNew(1, 0)
            })
            ...
            Items["PingValue"] = Instances:Create("TextLabel", {
                ...
                TextSize = 13,
                BackgroundColor3 = FromRGB(255, 255, 255)
            })  Items["PingValue"]:AddToTheme({TextColor3 = "Text"})

            Items["PingDescription"] = Instances:Create("TextLabel", {
                Parent = Items["PingCard"].Instance,
                Name = "\0",
                FontFace = Library.Font,
                TextColor3 = FromRGB(255, 255, 255),
                ...
                TextSize = 12,
                BackgroundColor3 = FromRGB(255, 255, 255)
            })  Items["PingDescription"]:AddToTheme({TextColor3 = "Text"})

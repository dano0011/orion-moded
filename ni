...
local Library do
    ...
    local function getAutoConfig()
        if isfile(ConfigStateFile) then
            return readfile(ConfigStateFile)
        end
    end
    ...
    Library.DeleteConfig = function(self, Config)
        ...
        if isfile(ConfigStateFile) and readfile(ConfigStateFile) == Config:gsub(".json","") then
    ...
    do -- Components
        Library.MakeWindow = function(self, Data)
            ...
            Window:SetOpen(true)
                if Window.GlobalChatOptions and Window.GlobalChatOptions.Enabled then
                task.defer(function()
        ...
        Library.Pages.GlobalChat = function(self)
            ...
            function ChatSystem:SendMessage(Avatar, Username, Message, IsLocalPlayer)
                Username = Toggle.Value and "Anonymous" or Username
                Avatar = Toggle.Value and getAsset("80377955535798.png") or Avatar
            ...
            Items["SendButton"]:Connect("MouseButton1Down", function()
                if ChatSystem:GetTypedMessage() == "" then
                    return
                end

...
local Library do
    ...
    Library.AttachExpressButton = function(self, Window)
        ...
        for _, GuiName in {"CustomScreenGui", "FPS_Ping_GUI"} do
        ...
        local Widgets = {
            ...
            LastSettings = {
                ...
                ShowFPS = self.ExpressExtraSettings.ShowFPS,
                ShowPing = self.ExpressExtraSettings.ShowPing,
        ...
        local function DestroyPerformanceGui()
            if Widgets.PerformanceGui then
                Widgets.PerformanceGui:Clean()
            end

            Widgets.PerformanceGui = nil
            Widgets.DisplayFrame = nil
            Widgets.StatsLabel = nil
            Widgets.IsStatsRunning = false
        end

        local function CreatePerformanceGui()
            if not (CheckSetting("ShowFPS") or CheckSetting("ShowPing")) then
                return false
            end

            if Widgets.PerformanceGui and Widgets.PerformanceGui.Instance and Widgets.PerformanceGui.Instance.Parent then
                return true
            end

            Widgets.PerformanceGui = Instances:Create("ScreenGui", {
                Parent = CoreGui,
                Name = "FPS_Ping_GUI",
                ResetOnSpawn = false,
            ...
            Widgets.DisplayFrame = Instances:Create("Frame", {
                Parent = Widgets.PerformanceGui.Instance,
                Size = UDim2New(0, 220, 0, 30),
                Position = UDim2New(0, 975, 1, -40),
            ...
            Instances:Create("UICorner", {
                Parent = Widgets.DisplayFrame.Instance,
                CornerRadius = UDimNew(0, 6)
            })
            ...
            Widgets.StatsLabel = Instances:Create("TextLabel", {
                Parent = Widgets.DisplayFrame.Instance,
                Size = UDim2New(1, 0, 1, 0),
                BackgroundTransparency = 1,
                TextColor3 = FromRGB(255, 255, 255),
            ...
            Widgets.DisplayFrame:MakeDraggable()
            return true
        end

        local function UpdatePerformanceStats()
            if Widgets.IsStatsRunning then
                return
            end

            if not CreatePerformanceGui() then
                return
            end

            Widgets.IsStatsRunning = true

            task.spawn(function()
                local LastText = nil

                while Widgets.IsStatsRunning and Widgets.PerformanceGui and Widgets.PerformanceGui.Instance and Widgets.PerformanceGui.Instance.Parent do
                    local Text = ""

                    if CheckSetting("ShowFPS") then
                        local Fps = Stats.Workspace.Heartbeat:GetValueString():match("^(%d+)")
                        Text = "📈 FPS: " .. (Fps or "0")
                    end

                    if CheckSetting("ShowPing") then
                        local Ping = MathFloor(Stats.Network.ServerStatsItem["Data Ping"]:GetValue() + 0.5)
                        if Text ~= "" then
                            Text = Text .. "   |   "
                        end
                    ...
                    if Text == "" then
                        DestroyPerformanceGui()
                        return
                    end

                    if Widgets.StatsLabel and Text ~= LastText then
                        Widgets.StatsLabel.Instance.Text = Text
                        LastText = Text
                    end
        ...
        local function CheckSettingsChanged()
            ...
            if Widgets.LastSettings.ShowFPS ~= Current.ShowFPS or Widgets.LastSettings.ShowPing ~= Current.ShowPing then
                if Current.ShowFPS or Current.ShowPing then
                    UpdatePerformanceStats()
                else
                    DestroyPerformanceGui()
                end

                Widgets.LastSettings.ShowFPS = Current.ShowFPS
                Widgets.LastSettings.ShowPing = Current.ShowPing
            end
        end
        ...
        Widgets.ImageLabel = Instances:Create("ImageLabel", {
            ...
            Size = UDim2New(1, 0, 1, 0),
            BackgroundTransparency = 1,
            Image = getAsset("118529184603611.png")
        })
        ...
        Widgets.MainButton.Instance.MouseButton1Click:Connect(function()
            ...
            if CheckSetting("ShowFPS") or CheckSetting("ShowPing") then
                if Widgets.PerformanceGui then
                    DestroyPerformanceGui()
                else
                    UpdatePerformanceStats()
    ...
    do -- Components
        Library.MakeWindow = function(self, Data)
            ...
            function Window:SetStreamerMode(Bool)
                if Bool then
                    Items["Avatar"].Instance.Image = StreamerContent
                    Items["Username"].Instance.Text = StreamerName

                    if Items["AccountAvatar"] then
                        Items["AccountAvatar"].Instance.Image = StreamerContent
                    end

                    if Items["AccountDisplayName"] then
                        Items["AccountDisplayName"].Instance.Text = StreamerName
                    end

                    if Items["AccountUsername"] then
                        Items["AccountUsername"].Instance.Text = "@" .. StreamerName
                    end
                else
                    Items["Avatar"].Instance.Image = Content
                    Items["Username"].Instance.Text = LocalPlayer.Name

                    if Items["AccountAvatar"] then
                        Items["AccountAvatar"].Instance.Image = Content
                    end

                    if Items["AccountDisplayName"] then
                        Items["AccountDisplayName"].Instance.Text = LocalPlayer.DisplayName
                    end

                    if Items["AccountUsername"] then
                        Items["AccountUsername"].Instance.Text = "@" .. LocalPlayer.Name
                    end
                end
            end
                    ...
                    if Playerlist.Player and Playerlist.Player ~= LocalPlayer then
                        ...
                        if localChar and targetChar then
                            ...
                            if localRoot and targetRoot then
                                local NewDistanceText = "Distance: " .. MathFloor((localRoot.Position - targetRoot.Position).Magnitude) .. " studs"
        ...
        Library.Pages.AddSection = function(self, Data)
            ...
            local Items = { } do 
                ...
                Items["Description"].Instance.Visible = false
                ...
                function Section:SetOpen(Bool)
                    ...
                    if Bool then
                        if Section.Description ~= "" then
                            Items["Description"].Instance.Visible = true
                        end

                        Items["Content"].Instance.Visible = true

                        if Section.Premium then
                            Items["IsPremium"].Instance.Visible = true
                            Items["PremiumText"].Instance.Visible = true
                        end
                    end
                    ...
                    local TargetHeight = Bool and (Items["Content"].Instance.Position.Y.Offset + Items["Content"].Instance.AbsoluteSize.Y + 8) or 35

                    Items["Section"].Instance.AutomaticSize = Enum.AutomaticSize.None
                    Items["Section"].Instance.Size = UDim2New(1, 0, 0, CurrentHeight)

                    SectionTween = TweenService:Create(Items["Section"].Instance, TweenInfo.new(Library.Tween.Time, Library.Tween.Style, Library.Tween.Direction), {
                        Size = UDim2New(1, 0, 0, TargetHeight)
                    })

                    SectionTween.Completed:Once(function()
                        if self.IsOpen ~= Bool then
                            return
                        end
        ...
        Library.Sections.AddToggle = function(self, Data)
            ...
            local Items = { } do
                ...
                UpdateToggleVisuals = function()
                    ...
                    Items["Indicator"].Instance.Size = IsCheckbox and UDim2New(0, 18, 0, 18) or UDim2New(0, 32, 0, 18)
                    Items["Text"].Instance.Position = IsCheckbox and UDim2New(0, 26, 0.5, 0) or UDim2New(0, 40, 0.5, 0)
    ...
    Library.CreateConfigsSection = function(self, Tab)
        ...
        ConfigsSection:AddButton({
            ...
            Callback = function()
                if ConfigSelected then
                    local Success, Message = Library:LoadConfig(readfile(Library.Folders.Configs .. "/" .. ConfigSelected..".json"))
    ...
    Library.CreateSettingsSection = function(self, Tab)
        ...
        SettingsSection:AddToggle({
            Name = "Show Button",
            Flag = "ShowButton",
            Default = Library.ExpressExtraSettings.ShowButton,
            Callback = function(Value)
                Library.ExpressExtraSettings.ShowButton = Value
            end
        })
        ...
        SettingsSection:AddToggle({
            Name = "Show FPS",
            Flag = "ShowFPS",
            Default = Library.ExpressExtraSettings.ShowFPS,
            Callback = function(Value)
                Library.ExpressExtraSettings.ShowFPS = Value
            end
        })
        ...
        SettingsSection:AddToggle({
            Name = "Show Ping",
            Flag = "ShowPing",
            Default = Library.ExpressExtraSettings.ShowPing,
            Callback = function(Value)
                Library.ExpressExtraSettings.ShowPing = Value
            end
        })
    end

...
local Library do
    ...
    do -- Components
        ...
        Library.Pages.Playerlist = function(self, Data)
            ...
            local function GetToolsPlayerNames()
                local Names = { }
            ...
            local function UpdateToolsPlayerDropdown(RefreshPlayers)
                if not ToolsPlayerDropdown then
                    return
                end

                local ShowDropdown = IsSettingsOpen and not Playerlist.Page.Active and not Playerlist.Window.IsMinimized
                if ToolsPlayerDropdown.IsOpen and not ShowDropdown then
                    ToolsPlayerDropdown:SetOpen(false)
                end

                if RefreshPlayers then
                    ToolsPlayerDropdown:Refresh(GetToolsPlayerNames(), true)
                end

                ToolsPlayerDropdown:SetVisibility(ShowDropdown)

                if Playerlist.Player and ToolsPlayerDropdown.OptionInstances[Playerlist.Player.Name] then
                    ToolsPlayerDropdown:SetText(Playerlist.Player.Name)
                else
                    ToolsPlayerDropdown:SetText("Select Player")
                end
            end
            ...
            ToolsPlayerDropdown = Playerlist:AddDropdown({
                ...
                Options = GetToolsPlayerNames,
                Callback = function(Name)
                    local Player = Players:FindFirstChild(Name)
                    local PlayerData = Playerlist.Players[Name]
                    if not Player then
                        return
                    end
                    ...
                    Items["PlayerUsername"].Instance.Text = Player.DisplayName .. " (@" .. Player.Name .. ")"
                    Items["PlayerUserID"].Instance.Text = "User ID: "..tostring(Player.UserId)

                    if Data.Callback then 
                        Library:SafeCall(Data.Callback, Playerlist.Player, PlayerData and PlayerData.PlayerStatus.Instance.Text, PlayerData and PlayerData.PlayerTeam.Instance.Text)
                    end
                end
            })

            UpdateToolsPlayerDropdown(true)

            Items["CloseTools"]:Connect("MouseButton1Click", CloseTools)
            Library:Connect(Playerlist.Page.Items["Page"].Instance:GetPropertyChangedSignal("Visible"), UpdateToolsPlayerDropdown)

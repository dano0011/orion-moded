    ...
    do -- Components
        ...
        Library.Pages.AddSection = function(self, Data)
            ...
            local Items = { } do 
                ...
                local function GetAnimatedSectionItems()
                    local AnimatedItems = { }

                    if Section.Description ~= "" then
                        TableInsert(AnimatedItems, Items["Description"].Instance)
                    end

                    TableInsert(AnimatedItems, Items["Content"].Instance)

                    for _, Value in Items["Content"].Instance:GetDescendants() do
                        TableInsert(AnimatedItems, Value)
                    end

                    if Section.Premium then
                        TableInsert(AnimatedItems, Items["IsPremium"].Instance)
                        TableInsert(AnimatedItems, Items["PremiumText"].Instance)
                    end

                    return AnimatedItems
                end

                local function UpdateSectionLayout(ShowContent)
                    local HasDescription = Section.Description ~= ""
                    local ContentVisible = ShowContent

                    if ContentVisible == nil then
                        ContentVisible = Section.IsOpen
                    end

                    Items["Description"].Instance.Visible = HasDescription and ContentVisible
                    Items["Content"].Instance.Visible = ContentVisible
                    ...
                    Items["Toggle"].Instance.Size = UDim2New(1, 0, 0, ContentVisible and Items["Content"].Instance.Position.Y.Offset or 35)

                    if Section.Premium then
                        Items["IsPremium"].Instance.Visible = ContentVisible
                        Items["PremiumText"].Instance.Visible = ContentVisible
                        Items["IsPremium"].Instance.Size = UDim2New(1, -4, 0, Items["Content"].Instance.AbsoluteSize.Y + 4)
                        Items["IsPremium"].Instance.Position = UDim2New(0, 2, 0, Items["Content"].Instance.Position.Y.Offset - 4)
                    end
                end
                ...
                function Section:SetOpen(Bool)
                    ...
                    local FadeToken = self._FadeToken
                    local AnimatedItems

                    UpdateSectionLayout(true)
                    AnimatedItems = GetAnimatedSectionItems()

                    for _, Item in AnimatedItems do
                        local TransparencyProperty = Tween:GetProperty(Item)

                        if TransparencyProperty then
                            if type(TransparencyProperty) == "table" then
                                for _, Property in TransparencyProperty do
                                    Tween:FadeItem(Item, Property, Bool, 0.16)
                                end
                            else
                                Tween:FadeItem(Item, TransparencyProperty, Bool, 0.16)
                            end
                        end
                    end
                    ...
                    if not Bool then
                        task.delay(0.16, function()
                            if not Library or self._FadeToken ~= FadeToken or self.IsOpen then
                                return
                            end
